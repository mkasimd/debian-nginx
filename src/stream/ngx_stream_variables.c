...
static ngx_int_t ngx_stream_variable_bytes(ngx_stream_session_t *s,
                                           ngx_stream_variable_value_t *v,
                                           uintptr_t data);
static ngx_int_t ngx_stream_variable_session_time(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
...
static ngx_int_t ngx_stream_variable_nginx_version(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
...
static ngx_int_t ngx_stream_variable_msec(ngx_stream_session_t *s,
                                          ngx_stream_variable_value_t *v,
                                          uintptr_t data);
static ngx_int_t ngx_stream_variable_time_iso8601(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);
...
static ngx_stream_variable_t ngx_stream_core_variables[] = {
    ...
    {ngx_string("session_time"), NULL, ngx_stream_variable_session_time, 0,
     NGX_STREAM_VAR_NOCACHEABLE, 0},

    {ngx_string("status"), NULL, ngx_stream_variable_status, 0,
     NGX_STREAM_VAR_NOCACHEABLE, 0},

    {ngx_string("connection"), NULL, ngx_stream_variable_connection, 0, 0, 0},

    {ngx_string("nginx_version"), NULL, ngx_stream_variable_nginx_version, 0, 0,
     0},
    ...
    {ngx_string("msec"), NULL, ngx_stream_variable_msec, 0,
     NGX_STREAM_VAR_NOCACHEABLE, 0},

    {ngx_string("time_iso8601"), NULL, ngx_stream_variable_time_iso8601, 0,
     NGX_STREAM_VAR_NOCACHEABLE, 0},

    {ngx_string("time_local"), NULL, ngx_stream_variable_time_local, 0,
     NGX_STREAM_VAR_NOCACHEABLE, 0},
...
ngx_int_t ngx_stream_get_variable_index(ngx_conf_t *cf, ngx_str_t *name) {
  ...
  if (v == NULL) {
    if (ngx_array_init(&cmcf->variables, cf->pool, 4,
                       sizeof(ngx_stream_variable_t)) != NGX_OK) {
      return NGX_ERROR;
    }
...
static ngx_int_t ngx_stream_variable_session_time(
  ...
  ms = (ngx_msec_int_t)((tp->sec - s->start_sec) * 1000 +
...
ngx_int_t ngx_stream_regex_exec(ngx_stream_session_t *s, ngx_stream_regex_t *re,
  ...
  if (rc < 0) {
    ngx_log_error(NGX_LOG_ALERT, s->connection->log, 0,
                  ngx_regex_exec_n " failed: %i on \"%V\" using \"%V\"", rc,
                  str, &re->name);
    return NGX_ERROR;
  }
