...
static ngx_int_t ngx_stream_script_add_var_code(ngx_stream_script_compile_t *sc,
                                                ngx_str_t *name);
#if (NGX_PCRE)
static ngx_int_t
...
size_t ngx_stream_complex_value_size(ngx_stream_session_t *s,
  ...
  if (size == (size_t)NGX_ERROR) {
    ngx_log_error(NGX_LOG_ERR, s->connection->log, 0, "invalid size \"%V\"",
                  &value);
    return default_value;
  }
...
static ngx_int_t ngx_stream_script_done(ngx_stream_script_compile_t *sc) {
  ...
  if (sc->complete_values) {
    code =
        ngx_stream_script_add_code(*sc->values, sizeof(uintptr_t), &sc->main);
    if (code == NULL) {
      return NGX_ERROR;
    }
...
ngx_stream_script_add_copy_code(ngx_stream_script_compile_t *sc,
  ...
  code = ngx_stream_script_add_code(
      *sc->lengths, sizeof(ngx_stream_script_copy_code_t), NULL);
  if (code == NULL) {
    return NGX_ERROR;
  }

  code->code =
      (ngx_stream_script_code_pt)(void *)ngx_stream_script_copy_len_code;
  code->len = len;
...
void ngx_stream_script_copy_code(ngx_stream_script_engine_t *e) {
  ...
  if (!e->skip) {
    e->pos =
        ngx_copy(p, e->ip + sizeof(ngx_stream_script_copy_code_t), code->len);
  }
...
static ngx_int_t ngx_stream_script_add_var_code(ngx_stream_script_compile_t *sc,
  ...
  code = ngx_stream_script_add_code(*sc->lengths,
                                    sizeof(ngx_stream_script_var_code_t), NULL);
  if (code == NULL) {
    return NGX_ERROR;
  }

  code->code =
      (ngx_stream_script_code_pt)(void *)ngx_stream_script_copy_var_len_code;
  code->index = (uintptr_t)index;

  code = ngx_stream_script_add_code(
      *sc->values, sizeof(ngx_stream_script_var_code_t), &sc->main);
  if (code == NULL) {
    return NGX_ERROR;
  }
...
ngx_stream_script_add_capture_code(ngx_stream_script_compile_t *sc,
  ...
  code = ngx_stream_script_add_code(
      *sc->lengths, sizeof(ngx_stream_script_copy_capture_code_t), NULL);
  if (code == NULL) {
    return NGX_ERROR;
  }

  code->code = (ngx_stream_script_code_pt)(
      void *)ngx_stream_script_copy_capture_len_code;
  code->n = 2 * n;

  code = ngx_stream_script_add_code(
      *sc->values, sizeof(ngx_stream_script_copy_capture_code_t), &sc->main);
  if (code == NULL) {
    return NGX_ERROR;
  }
...
ngx_stream_script_add_full_name_code(ngx_stream_script_compile_t *sc) {
  ...
  code = ngx_stream_script_add_code(
      *sc->lengths, sizeof(ngx_stream_script_full_name_code_t), NULL);
  if (code == NULL) {
    return NGX_ERROR;
  }

  code->code =
      (ngx_stream_script_code_pt)(void *)ngx_stream_script_full_name_len_code;
  code->conf_prefix = sc->conf_prefix;

  code = ngx_stream_script_add_code(
      *sc->values, sizeof(ngx_stream_script_full_name_code_t), &sc->main);
  if (code == NULL) {
    return NGX_ERROR;
  }
