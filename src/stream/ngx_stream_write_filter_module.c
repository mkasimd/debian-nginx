...
static ngx_int_t ngx_stream_write_filter(ngx_stream_session_t *s,
  ...
  if (ctx == NULL) {
    ctx =
        ngx_pcalloc(s->connection->pool, sizeof(ngx_stream_write_filter_ctx_t));
    if (ctx == NULL) {
      return NGX_ERROR;
    }
  ...
  *ll = NULL;
  ...
  if (size == 0 && !(c->buffered & NGX_LOWLEVEL_BUFFERED) &&
      !(last && c->need_last_buf) && !(c->type == SOCK_DGRAM && flush)) {
    if (last || flush || sync) {
      for (cl = *out; cl; /* void */) {
        ln = cl;
        cl = cl->next;
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_STREAM, c->log, 0, "stream write filter %p",
                 chain);
