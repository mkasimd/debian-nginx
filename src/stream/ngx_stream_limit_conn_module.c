...
static ngx_int_t ngx_stream_limit_conn_status_variable(
...
static ngx_command_t ngx_stream_limit_conn_commands[] = {
    ...
    {ngx_string("limit_conn_dry_run"),
     ...
     offsetof(ngx_stream_limit_conn_conf_t, dry_run), NULL},
...
static ngx_str_t ngx_stream_limit_conn_status[] = {
    ngx_string("PASSED"), ngx_string("REJECTED"),
  ...
  for (i = 0; i < lccf->limits.nelts; i++) {
    ...
    if (node == NULL) {
      ...
      n = offsetof(ngx_rbtree_node_t, color) +
          offsetof(ngx_stream_limit_conn_node_t, data) + key.len;
...
static ngx_int_t ngx_stream_limit_conn_init_zone(ngx_shm_zone_t *shm_zone,
                                                 void *data) {
  ...
  ctx->sh = ngx_slab_alloc(ctx->shpool, sizeof(ngx_stream_limit_conn_shctx_t));
  if (ctx->sh == NULL) {
    return NGX_ERROR;
  }
...
static char *ngx_stream_limit_conn_zone(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  shm_zone =
      ngx_shared_memory_add(cf, &name, size, &ngx_stream_limit_conn_module);
  if (shm_zone == NULL) {
    return NGX_CONF_ERROR;
  }
...
static char *ngx_stream_limit_conn(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  shm_zone =
      ngx_shared_memory_add(cf, &value[1], 0, &ngx_stream_limit_conn_module);
  if (shm_zone == NULL) {
    return NGX_CONF_ERROR;
  }
  ...
  if (limits == NULL) {
    if (ngx_array_init(&lccf->limits, cf->pool, 1,
                       sizeof(ngx_stream_limit_conn_limit_t)) != NGX_OK) {
      return NGX_CONF_ERROR;
    }
  }
