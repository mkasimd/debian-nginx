...
static ngx_command_t ngx_mail_core_commands[] = {
    ...
    {ngx_string("server_name"),
     ...
     offsetof(ngx_mail_core_srv_conf_t, server_name), NULL},
    ...
    {ngx_string("resolver_timeout"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_core_srv_conf_t, resolver_timeout), NULL},
...
static void *ngx_mail_core_create_main_conf(ngx_conf_t *cf) {
  ...
  if (ngx_array_init(&cmcf->servers, cf->pool, 4,
                     sizeof(ngx_mail_core_srv_conf_t *)) != NGX_OK) {
    return NULL;
  }
...
static char *ngx_mail_core_merge_srv_conf(ngx_conf_t *cf, void *parent,
  ...
  if (conf->protocol == NULL) {
    ngx_log_error(NGX_LOG_EMERG, cf->log, 0,
                  "unknown mail protocol for server in %s:%ui", conf->file_name,
                  conf->line);
...
static char *ngx_mail_core_listen(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  for (i = 2; i < cf->args->nelts; i++) {
    ...
    if (ngx_strncmp(value[i].data, "ipv6only=o", 10) == 0) {
      ...
      if (ngx_strcmp(&value[i].data[10], "n") == 0) {
        ...
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "invalid ipv6only flags \"%s\"", &value[i].data[9]);
        return NGX_CONF_ERROR;
      }
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "the invalid \"%V\" parameter",
                       &value[i]);
    return NGX_CONF_ERROR;
  }
  ...
  for (n = 0; n < u.naddrs; n++) {
    ...
    for (i = 0; i < cmcf->listen.nelts - u.naddrs + n; i++) {
      ...
      if (ngx_cmp_sockaddr(als[i].sockaddr, als[i].socklen, ls[n].sockaddr,
                           ls[n].socklen, 1) != NGX_OK) {
        continue;
      }
...
static char *ngx_mail_core_protocol(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unknown protocol \"%V\"",
                     &value[1]);
  return NGX_CONF_ERROR;
}
