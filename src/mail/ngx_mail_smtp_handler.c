...
static void ngx_mail_smtp_resolve_addr_handler(ngx_resolver_ctx_t *ctx) {
  ...
  if (ctx->state) {
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "%V could not be resolved (%i: %s)",
                  &c->addr_text, ctx->state, ngx_resolver_strerror(ctx->state));
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "address resolved: %V",
                 &s->host);
  ...
  if (ctx->state) {
...
#if (NGX_DEBUG)
    {
      ...
      for (i = 0; i < ctx->naddrs; i++) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "name was resolved to %V",
                       &addr);
      }
    }
#endif
...
static void ngx_mail_smtp_greeting(ngx_mail_session_t *s, ngx_connection_t *c) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "smtp greeting for \"%V\"",
                 &s->host);
...
static void ngx_mail_smtp_invalid_pipelining(ngx_event_t *rev) {
  ...
  if (rev->timedout) {
    ...
    if (ngx_mail_smtp_discard_command(
            s, c, "client was rejected before greeting: \"%V\"") != NGX_OK) {
      return;
    }
...
static ngx_int_t ngx_mail_smtp_helo(ngx_mail_session_t *s,
                                    ngx_connection_t *c) {
  ngx_str_t *arg;
  ngx_mail_smtp_srv_conf_t *sscf;
...
static ngx_int_t ngx_mail_smtp_auth(ngx_mail_session_t *s,
                                    ngx_connection_t *c) {
  ngx_int_t rc;
  ngx_mail_core_srv_conf_t *cscf;
  ngx_mail_smtp_srv_conf_t *sscf;
...
static ngx_int_t ngx_mail_smtp_mail(ngx_mail_session_t *s,
                                    ngx_connection_t *c) {
  ngx_str_t *arg, cmd;
  ngx_mail_smtp_srv_conf_t *sscf;
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "smtp mail from:\"%V\"",
                 &s->smtp_from);
...
static ngx_int_t ngx_mail_smtp_rcpt(ngx_mail_session_t *s,
                                    ngx_connection_t *c) {
  ngx_str_t *arg, cmd;
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "smtp rcpt to:\"%V\"",
                 &s->smtp_to);
...
static ngx_int_t ngx_mail_smtp_rset(ngx_mail_session_t *s,
                                    ngx_connection_t *c) {
  ngx_str_null(&s->smtp_from);
  ngx_str_null(&s->smtp_to);
  ngx_str_set(&s->out, smtp_ok);
