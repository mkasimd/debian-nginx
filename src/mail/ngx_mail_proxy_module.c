...
static ngx_command_t ngx_mail_proxy_commands[] = {
    ...
    {ngx_string("proxy_buffer"),
     ...
     ngx_conf_set_size_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_proxy_conf_t, buffer_size), NULL},

    {ngx_string("proxy_timeout"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_proxy_conf_t, timeout), NULL},

    {ngx_string("proxy_pass_error_message"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_proxy_conf_t, pass_error_message), NULL},
    ...
    {ngx_string("proxy_smtp_auth"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_proxy_conf_t, smtp_auth), NULL},

    {ngx_string("proxy_protocol"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_proxy_conf_t, proxy_protocol), NULL},
...
static void ngx_mail_proxy_imap_handler(ngx_event_t *rev) {
  ...
  case ngx_imap_start:
    ...
    line.len =
        ngx_sprintf(line.data, "%VLOGIN {%uz}" CRLF, &s->tag, s->login.len) -
        line.data;
  ...
  case ngx_imap_login:
    ...
    line.len =
        ngx_sprintf(line.data, "%V {%uz}" CRLF, &s->login, s->passwd.len) -
        line.data;
...
static void ngx_mail_proxy_smtp_handler(ngx_event_t *rev) {
  ...
  case ngx_smtp_start:
    ...
    p = ngx_cpymem(line.data, ((s->esmtp || pcf->xclient) ? "EHLO " : "HELO "),
                   sizeof("HELO ") - 1);
  ...
  case ngx_smtp_xclient_helo:
    ...
    line.len =
        ngx_sprintf(line.data, ((s->esmtp) ? "EHLO %V" CRLF : "HELO %V" CRLF),
                    &s->smtp_helo) -
        line.data;
  ...
  case ngx_smtp_xclient_auth:
    ngx_log_debug0(NGX_LOG_DEBUG_MAIL, rev->log, 0, "mail proxy send auth");
    ...
    auth.len =
        ngx_sprintf(auth.data, "%Z%V%Z%V", &s->login, &s->passwd) - auth.data;

    line.len =
        sizeof("AUTH PLAIN " CRLF) - 1 + ngx_base64_encoded_length(auth.len);
    ...
    encoded.data =
        ngx_cpymem(line.data, "AUTH PLAIN ", sizeof("AUTH PLAIN ") - 1);
...
static ngx_int_t ngx_mail_proxy_read_response(ngx_mail_session_t *s,
  ...
  if (*(b->last - 2) != CR || *(b->last - 1) != LF) {
    if (b->last == b->end) {
      *(b->last - 1) = '\0';
      ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
                    "upstream sent too long response line: \"%s\"", b->pos);
      return NGX_ERROR;
    }
...
static void ngx_mail_proxy_handler(ngx_event_t *ev) {
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_MAIL, ev->log, 0,
                 "mail proxy handler: %ui, #%d > #%d", do_write, src->fd,
                 dst->fd);
