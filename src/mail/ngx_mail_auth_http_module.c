...
static ngx_command_t ngx_mail_auth_http_commands[] = {
    ...
    {ngx_string("auth_http_timeout"),
     NGX_MAIL_MAIN_CONF | NGX_MAIL_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_auth_http_conf_t, timeout), NULL},
    ...
    {ngx_string("auth_http_pass_client_cert"),
     ...
     ngx_conf_set_flag_slot, NGX_MAIL_SRV_CONF_OFFSET,
     offsetof(ngx_mail_auth_http_conf_t, pass_client_cert), NULL},
...
static ngx_str_t ngx_mail_auth_http_method[] = {
    ngx_string("plain"), ngx_string("plain"),    ngx_string("plain"),
    ngx_string("apop"),  ngx_string("cram-md5"), ngx_string("external"),
    ngx_string("none")};
    ...
    if (rc == NGX_OK) {
...
#if (NGX_DEBUG)
      {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_MAIL, s->connection->log, 0,
                       "mail auth http header: \"%V: %V\"", &key, &value);
      }
#endif
      ...
      if (len == sizeof("Auth-Status") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-Status",
                          sizeof("Auth-Status") - 1) == 0) {
        len = ctx->header_end - ctx->header_start;

        if (len == 2 && ctx->header_start[0] == 'O' &&
            ctx->header_start[1] == 'K') {
          continue;
        }

        if (len == 4 && ctx->header_start[0] == 'W' &&
            ctx->header_start[1] == 'A' && ctx->header_start[2] == 'I' &&
      ...
      if (len == sizeof("Auth-Server") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-Server",
      ...
      if (len == sizeof("Auth-Port") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-Port",
      ...
      if (len == sizeof("Auth-User") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-User",
      ...
      if (len == sizeof("Auth-Pass") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-Pass",
      ...
      if (len == sizeof("Auth-Wait") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-Wait",
      ...
      if (len == sizeof("Auth-Error-Code") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Auth-Error-Code",
                          sizeof("Auth-Error-Code") - 1) == 0) {
    ...
    if (rc == NGX_DONE) {
      ...
      if (ctx->err.len) {
        ...
        if (s->protocol == NGX_MAIL_SMTP_PROTOCOL) {
          ...
          ctx->err.len =
              ctx->errcode.len + ctx->errmsg.len + sizeof(" " CRLF) - 1;
      ...
      rc = ngx_parse_addr(s->connection->pool, peer, ctx->addr.data,
                          ctx->addr.len);
...
ngx_mail_auth_http_create_request(ngx_mail_session_t *s, ngx_pool_t *pool,
  ...
  len = sizeof("GET ") - 1 + ahcf->uri.len + sizeof(" HTTP/1.0" CRLF) - 1 +
        ...
        sizeof("Auth-Salt: ") - 1 + s->salt.len + sizeof("Auth-Protocol: ") -
        1 + cscf->protocol->name.len + sizeof(CRLF) - 1 +
        sizeof("Auth-Login-Attempt: ") - 1 + NGX_INT_T_LEN + sizeof(CRLF) - 1 +
        sizeof("Client-IP: ") - 1 + s->connection->addr_text.len +
        sizeof(CRLF) - 1 + sizeof("Client-Host: ") - 1 + s->host.len +
        sizeof(CRLF) - 1 + ahcf->header.len + sizeof(CRLF) - 1;

  if (c->proxy_protocol) {
    len += sizeof("Proxy-Protocol-Addr: ") - 1 +
           c->proxy_protocol->src_addr.len + sizeof(CRLF) - 1 +
           sizeof("Proxy-Protocol-Port: ") - 1 + sizeof("65535") - 1 +
           sizeof(CRLF) - 1 + sizeof("Proxy-Protocol-Server-Addr: ") - 1 +
           c->proxy_protocol->dst_addr.len + sizeof(CRLF) - 1 +
           sizeof("Proxy-Protocol-Server-Port: ") - 1 + sizeof("65535") - 1 +
           sizeof(CRLF) - 1;
  }

  if (s->auth_method == NGX_MAIL_AUTH_NONE) {
    len += sizeof("Auth-SMTP-Helo: ") - 1 + s->smtp_helo.len + sizeof(CRLF) -
           1 + sizeof("Auth-SMTP-From: ") - 1 + s->smtp_from.len +
           sizeof(CRLF) - 1 + sizeof("Auth-SMTP-To: ") - 1 + s->smtp_to.len +
           sizeof(CRLF) - 1;
  }
  ...
  if (c->ssl) {
    len += sizeof("Auth-SSL: on" CRLF) - 1 + sizeof("Auth-SSL-Verify: ") - 1 +
           verify.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Subject: ") - 1 +
           subject.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Issuer: ") - 1 +
           issuer.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Serial: ") - 1 +
           serial.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Fingerprint: ") -
           1 + fingerprint.len + sizeof(CRLF) - 1 + sizeof("Auth-SSL-Cert: ") -
           1 + cert.len + sizeof(CRLF) - 1;
  }
  ...
  *b->last++ = LF;
  ...
  b->last = ngx_cpymem(b->last, ngx_mail_auth_http_method[s->auth_method].data,
  ...
  *b->last++ = LF;
  ...
  b->last =
      ngx_cpymem(b->last, "Auth-Protocol: ", sizeof("Auth-Protocol: ") - 1);
  b->last =
      ngx_cpymem(b->last, cscf->protocol->name.data, cscf->protocol->name.len);
  *b->last++ = CR;
  *b->last++ = LF;

  b->last =
      ngx_sprintf(b->last, "Auth-Login-Attempt: %ui" CRLF, s->login_attempt);
  ...
  *b->last++ = LF;
  ...
  if (c->proxy_protocol) {
    b->last = ngx_cpymem(
        b->last, "Proxy-Protocol-Addr: ", sizeof("Proxy-Protocol-Addr: ") - 1);
  ...
  if (s->auth_method == NGX_MAIL_AUTH_NONE) {
    ...
    b->last =
        ngx_cpymem(b->last, "Auth-SMTP-Helo: ", sizeof("Auth-SMTP-Helo: ") - 1);
    b->last = ngx_copy(b->last, s->smtp_helo.data, s->smtp_helo.len);
    *b->last++ = CR;
    *b->last++ = LF;

    b->last =
        ngx_cpymem(b->last, "Auth-SMTP-From: ", sizeof("Auth-SMTP-From: ") - 1);
    b->last = ngx_copy(b->last, s->smtp_from.data, s->smtp_from.len);
    *b->last++ = CR;
    *b->last++ = LF;

    b->last =
        ngx_cpymem(b->last, "Auth-SMTP-To: ", sizeof("Auth-SMTP-To: ") - 1);
  ...
  if (c->ssl) {
    ...
    if (verify.len) {
      b->last = ngx_cpymem(
          b->last, "Auth-SSL-Verify: ", sizeof("Auth-SSL-Verify: ") - 1);
    ...
    if (subject.len) {
      b->last = ngx_cpymem(
          b->last, "Auth-SSL-Subject: ", sizeof("Auth-SSL-Subject: ") - 1);
    ...
    if (issuer.len) {
      b->last = ngx_cpymem(
          b->last, "Auth-SSL-Issuer: ", sizeof("Auth-SSL-Issuer: ") - 1);
    ...
    if (serial.len) {
      b->last = ngx_cpymem(
          b->last, "Auth-SSL-Serial: ", sizeof("Auth-SSL-Serial: ") - 1);
    ...
    if (cert.len) {
      b->last =
          ngx_cpymem(b->last, "Auth-SSL-Cert: ", sizeof("Auth-SSL-Cert: ") - 1);
  ...
  *b->last++ = LF;
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_MAIL, s->connection->log, 0,
                 "mail auth http header:%N\"%*s\"", (size_t)(b->last - b->pos),
                 b->pos);
#endif
...
static char *ngx_mail_auth_http(ngx_conf_t *cf, ngx_command_t *cmd,
                                void *conf) {
  ...
  if (ngx_parse_url(cf->pool, &u) != NGX_OK) {
    if (u.err) {
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in auth_http \"%V\"", u.err,
                         &u.url);
    }
