...
ngx_int_t ngx_mail_salt(ngx_mail_session_t *s, ngx_connection_t *c,
                        ngx_mail_core_srv_conf_t *cscf) {
  s->salt.data =
      ngx_pnalloc(c->pool, sizeof(" <18446744073709551616.@>" CRLF) - 1 +
                               NGX_TIME_T_LEN + cscf->server_name.len);
  if (s->salt.data == NULL) {
    return NGX_ERROR;
  }

  s->salt.len = ngx_sprintf(s->salt.data, "<%ul.%T@%V>" CRLF, ngx_random(),
                            ngx_time(), &cscf->server_name) -
                s->salt.data;
...
ngx_int_t ngx_mail_auth_plain(ngx_mail_session_t *s, ngx_connection_t *c,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "mail auth plain: \"%V\"",
                 &arg[n]);
#endif
...
ngx_int_t ngx_mail_auth_cram_md5_salt(ngx_mail_session_t *s,
                                      ngx_connection_t *c, char *prefix,
                                      size_t len) {
  u_char *p;
  ngx_str_t salt;
  ngx_uint_t n;
...
ngx_int_t ngx_mail_auth_cram_md5(ngx_mail_session_t *s, ngx_connection_t *c) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "mail auth cram-md5: \"%V\"",
                 &arg[0]);
...
ngx_int_t ngx_mail_auth_external(ngx_mail_session_t *s, ngx_connection_t *c,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "mail auth external: \"%V\"",
                 &arg[n]);
...
void ngx_mail_close_connection(ngx_connection_t *c) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_MAIL, c->log, 0, "close mail connection: %d",
                 c->fd);
...
u_char *ngx_mail_log_error(ngx_log_t *log, u_char *buf, size_t len) {
  ...
  p = ngx_snprintf(buf, len, "%s, server: %V",
                   s->starttls ? " using starttls" : "", s->addr_text);
  len -= p - buf;
  buf = p;
