...
static ngx_int_t ngx_event_pipe_read_upstream(ngx_event_pipe_t *p) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe read upstream: %d",
  ...
  for (;;) {
    ...
    if (p->preread_bufs) {
      ...
      if (p->free_raw_bufs) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe temp offset: %O",
                       p->temp_file->offset);
  ...
  for (cl = p->busy; cl; cl = cl->next) {
    ngx_log_debug8(NGX_LOG_DEBUG_EVENT, p->log, 0,
                   ...
                   "%p, pos %p, size: %z "
                   "file: %O, size: %O",
                   (cl->buf->shadow ? 1 : 0), cl->buf->temporary,
                   cl->buf->in_file, cl->buf->start, cl->buf->pos,
                   cl->buf->last - cl->buf->pos, cl->buf->file_pos,
...
static ngx_int_t ngx_event_pipe_write_to_downstream(ngx_event_pipe_t *p) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe write downstream: %d",
  ...
  for (;;) {
    ...
    if (downstream->data != p->output_ctx || !downstream->write->ready ||
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe write busy: %uz",
                   bsize);
    ...
    for (;;) {
      if (p->out) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, p->log, 0,
                       "pipe write buf ls:%d %p %z", cl->buf->last_shadow,
  ...
  flush:
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe write: out:%p, f:%ui",
                   out, flush);
...
static ngx_int_t ngx_event_pipe_write_chain_to_temp_file(ngx_event_pipe_t *p) {
  ...
  if (!p->cacheable) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe offset: %O",
                   p->temp_file->offset);
...
free:
  ...
  for (last_free = &p->free_raw_bufs; *last_free != NULL;
