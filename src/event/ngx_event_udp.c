...
void ngx_event_recvmsg(ngx_event_t *ev) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "recvmsg on %V, ready: %d",
  ...
  do {
    ...
    if (c) {
      ...
      if (c->log->log_level & NGX_LOG_DEBUG_EVENT) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "recvmsg: fd:%d n:%z",
                       c->fd, n);
    ...
    *log = ls->log;
    ...
    if (ls->addr_ntop) {
      ...
      c->addr_text.len = ngx_sock_ntop(
          c->sockaddr, c->socklen, c->addr_text.data, ls->addr_text_max_len, 0);
...
#if (NGX_DEBUG)
    {
      ...
      if (log->log_level & NGX_LOG_DEBUG_EVENT) {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_EVENT, log, 0,
                       "*%uA recvmsg: %V fd:%d n:%z", c->number, &addr, c->fd,
                       n);
      }
    }
#endif
...
void ngx_udp_rbtree_insert_value(ngx_rbtree_node_t *temp,
  ...
  for (;;) {
    ...
    if (node->key < temp->key) {
      ...
      rc = ngx_cmp_sockaddr(c->sockaddr, c->socklen, ct->sockaddr, ct->socklen,
                            1);
...
ngx_lookup_udp_connection(ngx_listening_t *ls, struct sockaddr *sockaddr,
  ...
  while (node != sentinel) {
    ...
    if (rc == 0 && ls->wildcard) {
      rc = ngx_cmp_sockaddr(local_sockaddr, local_socklen, c->local_sockaddr,
                            c->local_socklen, 1);
    }
...
void ngx_delete_udp_connection(void *data) { return; }
