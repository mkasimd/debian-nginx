...
static ngx_command_t ngx_devpoll_commands[] = {
    ...
    {ngx_string("devpoll_changes"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_devpoll_conf_t, changes), NULL},

    {ngx_string("devpoll_events"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_devpoll_conf_t, events), NULL},
...
ngx_module_t ngx_devpoll_module = {NGX_MODULE_V1,
...
static ngx_int_t ngx_devpoll_process_events(ngx_cycle_t *cycle,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "devpoll timer: %M",
                 timer);
  ...
  for (i = 0; i < events; i++) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "devpoll: fd:%d, ev:%04Xd, rev:%04Xd", fd,
                   event_list[i].events, revents);

    if (revents & (POLLERR | POLLHUP | POLLNVAL)) {
      ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                     "ioctl(DP_POLL) error fd:%d ev:%04Xd rev:%04Xd", fd,
                     event_list[i].events, revents);
    }
