...
ngx_module_t ngx_select_module = {NGX_MODULE_V1,
...
static ngx_int_t ngx_select_init(ngx_cycle_t *cycle, ngx_msec_t timer) {
  ...
  if (ngx_process >= NGX_PROCESS_WORKER || cycle->old_cycle == NULL ||
    ...
    index =
        ngx_alloc(sizeof(ngx_event_t *) * 2 * cycle->connection_n, cycle->log);
    if (index == NULL) {
      return NGX_ERROR;
    }
...
static ngx_int_t ngx_select_process_events(ngx_cycle_t *cycle, ngx_msec_t timer,
  ...
  if (max_fd == -1) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "change max_fd: %i",
                   max_fd);
  }
  ...
  for (i = 0; i < nevents; i++) {
    ...
    if (ev->write) {
      if (FD_ISSET(c->fd, &work_write_fd_set)) {
        found = 1;
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "select write %d",
                       c->fd);
      }
      ...
      if (FD_ISSET(c->fd, &work_read_fd_set)) {
        found = 1;
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "select read %d",
                       c->fd);
      }
    }
  ...
  if (ready != nready) {
    ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "select ready != events: %d:%d",
                  ready, nready);
