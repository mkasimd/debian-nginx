...
int timer_delete(timer_t timerid) { return -1; }
...
static ngx_command_t ngx_eventport_commands[] = {
    ...
    {ngx_string("eventport_events"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_eventport_conf_t, events), NULL},
...
static ngx_int_t ngx_eventport_add_event(ngx_event_t *ev, ngx_int_t event,
  ...
  if (port_associate(ep, PORT_SOURCE_FD, c->fd, events,
                     (void *)((uintptr_t)ev | ev->instance)) == -1) {
...
static ngx_int_t ngx_eventport_process_events(ngx_cycle_t *cycle,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "eventport timer: %M",
                 timer);
