...
static ngx_command_t ngx_epoll_commands[] = {
    ...
    {ngx_string("epoll_events"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_epoll_conf_t, events), NULL},

    {ngx_string("worker_aio_requests"), NGX_EVENT_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, 0, offsetof(ngx_epoll_conf_t, aio_requests), NULL},
...
static void ngx_epoll_aio_init(ngx_cycle_t *cycle, ngx_epoll_conf_t *epcf) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "eventfd: %d",
                 ngx_eventfd);
...
static ngx_int_t ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer) {
  ...
  if (nevents < epcf->events) {
    ...
    event_list =
        ngx_alloc(sizeof(struct epoll_event) * epcf->events, cycle->log);
    if (event_list == NULL) {
      return NGX_ERROR;
    }
  }
...
static void ngx_epoll_notify_handler(ngx_event_t *ev) {
  ...
  if (++ev->index == NGX_MAX_UINT32_VALUE) {
    ...
    if ((size_t)n != sizeof(uint64_t)) {
      ngx_log_error(NGX_LOG_ALERT, ev->log, err, "read() eventfd %d failed",
                    notify_fd);
    }
  }
...
static ngx_int_t ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                 "epoll add event: fd:%d op:%d ev:%08XD", c->fd, op, ee.events);

  if (epoll_ctl(ep, op, c->fd, &ee) == -1) {
    ngx_log_error(NGX_LOG_ALERT, ev->log, ngx_errno, "epoll_ctl(%d, %d) failed",
                  op, c->fd);
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_epoll_del_event(ngx_event_t *ev, ngx_int_t event,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                 "epoll del event: fd:%d op:%d ev:%08XD", c->fd, op, ee.events);
...
static ngx_int_t ngx_epoll_del_connection(ngx_connection_t *c,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "epoll del connection: fd:%d",
                 c->fd);
  ...
  if (epoll_ctl(ep, op, c->fd, &ee) == -1) {
    ngx_log_error(NGX_LOG_ALERT, c->log, ngx_errno, "epoll_ctl(%d, %d) failed",
                  op, c->fd);
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer,
  ...
  for (i = 0; i < events; i++) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "epoll: fd:%d ev:%04XD d:%p", c->fd, revents,
    ...
    if (events > 0) {
      ...
      for (i = 0; i < events; i++) {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_EVENT, ev->log, 0,
                       "io_event: %XL %XL %L %L", event[i].data, event[i].obj,
