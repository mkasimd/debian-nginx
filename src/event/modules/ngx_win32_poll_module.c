...
ngx_module_t ngx_poll_module = {NGX_MODULE_V1,
...
static ngx_int_t ngx_poll_init(ngx_cycle_t *cycle, ngx_msec_t timer) {
  ...
  if (ngx_process >= NGX_PROCESS_WORKER || cycle->old_cycle == NULL ||
...
static ngx_int_t ngx_poll_add_event(ngx_event_t *ev, ngx_int_t event,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "poll add event: fd:%d ev:%i",
                 c->fd, event);
  ...
  if (e == NULL || e->index == NGX_INVALID_INDEX) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ev->log, 0, "poll add index: %i",
                   e->index);
...
static ngx_int_t ngx_poll_del_event(ngx_event_t *ev, ngx_int_t event,
  ...
  if (ev->index == NGX_INVALID_INDEX) {
    ngx_log_error(NGX_LOG_ALERT, ev->log, 0,
                  "poll event fd:%d ev:%i is already deleted", c->fd, event);
    return NGX_OK;
  }
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "poll del event: fd:%d ev:%i",
                 c->fd, event);
  ...
  if (e == NULL || e->index == NGX_INVALID_INDEX) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ev->log, 0, "poll del index: %i",
                   e->index);
...
static ngx_int_t ngx_poll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer,
  ...
  if (cycle->log->log_level & NGX_LOG_DEBUG_ALL) {
    for (i = 0; i < nevents; i++) {
      ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                     "poll: %ui: fd:%d ev:%04Xd", i, event_list[i].fd,
                     event_list[i].events);
    }
  }
#endif
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "poll ready %d of %ui",
                 ready, nevents);
  ...
  for (i = 0; i < nevents && ready; i++) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "poll: %ui: fd:%d ev:%04Xd rev:%04Xd", i, event_list[i].fd,
    ...
    if (revents & POLLNVAL) {
      ngx_log_error(NGX_LOG_ALERT, cycle->log, 0,
                    "poll() error fd:%d ev:%04Xd rev:%04Xd", event_list[i].fd,
                    event_list[i].events, revents);
    }
