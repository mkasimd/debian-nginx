...
ngx_int_t ngx_ssl_init(ngx_log_t *log) {
  ...
  ngx_ssl_next_certificate_index =
      X509_get_ex_new_index(0, NULL, NULL, NULL, NULL);
...
ngx_int_t ngx_ssl_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *cert,
  ...
  if (x509 == NULL) {
    if (err != NULL) {
      ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                    "cannot load certificate \"%s\": %s", cert->data, err);
    }
  ...
  if (X509_set_ex_data(
          x509, ngx_ssl_next_certificate_index,
  ...
  if (pkey == NULL) {
    if (err != NULL) {
      ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                    "cannot load certificate key \"%s\": %s", key->data, err);
    }
...
ngx_int_t ngx_ssl_connection_certificate(ngx_connection_t *c, ngx_pool_t *pool,
  ...
  if (SSL_use_certificate(c->ssl->connection, x509) == 0) {
    ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "SSL_use_certificate(\"%s\") failed",
                  cert->data);
    X509_free(x509);
    sk_X509_pop_free(chain, X509_free);
    return NGX_ERROR;
  ...
  if (SSL_set0_chain(c->ssl->connection, chain) == 0) {
    ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "SSL_set0_chain(\"%s\") failed",
                  cert->data);
    sk_X509_pop_free(chain, X509_free);
    return NGX_ERROR;
  }
  ...
  if (SSL_use_PrivateKey(c->ssl->connection, pkey) == 0) {
    ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "SSL_use_PrivateKey(\"%s\") failed",
                  key->data);
    EVP_PKEY_free(pkey);
    return NGX_ERROR;
  }
...
ngx_int_t ngx_ssl_ciphers(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *ciphers,
  ...
  if (SSL_CTX_set_cipher_list(ssl->ctx, (char *)ciphers->data) == 0) {
    ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                  "SSL_CTX_set_cipher_list(\"%V\") failed", ciphers);
    return NGX_ERROR;
  }
...
ngx_int_t ngx_ssl_client_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl,
  ...
  if (SSL_CTX_load_verify_locations(ssl->ctx, (char *)cert->data, NULL) == 0) {
    ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                  "SSL_CTX_load_verify_locations(\"%s\") failed", cert->data);
    return NGX_ERROR;
  }
...
static int ngx_ssl_verify_callback(int ok, X509_STORE_CTX *x509_store) {
  ...
  ngx_log_debug5(NGX_LOG_DEBUG_EVENT, c->log, 0,
                 ...
                 "subject:\"%s\", issuer:\"%s\"",
                 ok, err, depth, subject ? subject : "(none)",
                 issuer ? issuer : "(none)");
...
ngx_int_t ngx_ssl_dhparam(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *file) {
  ...
  if (bio == NULL) {
    ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "BIO_new_file(\"%s\") failed",
                  file->data);
    return NGX_ERROR;
  }
...
ngx_int_t ngx_ssl_client_session_cache(ngx_conf_t *cf, ngx_ssl_t *ssl,
  ...
  SSL_CTX_set_session_cache_mode(ssl->ctx, SSL_SESS_CACHE_CLIENT |
...
static ngx_int_t ngx_ssl_try_early_data(ngx_connection_t *c) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "SSL_read_early_data: %d, %uz",
                 n, readbytes);
...
static void ngx_ssl_handshake_log(ngx_connection_t *c) {
  ...
  if (cipher) {
    ...
    *d = '\0';
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "SSL: %s, cipher: \"%s\"",
    ...
    if (c->ssl->last == NGX_OK) {
      ...
      if (size == 0) {
        ...
        if (c->read->available >= 0) {
          ...
          ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "SSL_read: avail:%d",
                         c->read->available);
...
#endif
        || err == NGX_EPIPE || err == NGX_ENOTCONN || err == NGX_ETIMEDOUT ||
        err == NGX_ECONNREFUSED || err == NGX_ENETDOWN ||
...
ngx_int_t ngx_ssl_session_cache(ngx_ssl_t *ssl, ngx_str_t *sess_ctx,
  ...
  if (builtin_session_cache == NGX_SSL_NONE_SCACHE) {
    ...
    SSL_CTX_set_session_cache_mode(
        ssl->ctx, SSL_SESS_CACHE_SERVER | SSL_SESS_CACHE_NO_AUTO_CLEAR |
...
static int ngx_ssl_new_session(ngx_ssl_conn_t *ssl_conn,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                 "ssl new session: %08XD:%ud:%d", hash, session_id_length, len);
...
failed:
  ...
  ngx_log_error(NGX_LOG_ALERT, c->log, 0, "could not allocate new session%s",
                shpool->log_ctx);
...
static ngx_ssl_session_t *ngx_ssl_get_cached_session(ngx_ssl_conn_t *ssl_conn,
  ...
  *copy = 0;
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl get session: %08XD:%d",
                 hash, len);

  shm_zone =
      SSL_CTX_get_ex_data(c->ssl->session_ctx, ngx_ssl_session_cache_index);
  ...
  while (node != sentinel) {
    ...
    rc = ngx_memn2cmp((u_char *)(uintptr_t)id, sess_id->id, (size_t)len,
                      (size_t)node->data);
...
static void ngx_ssl_session_rbtree_insert_value(ngx_rbtree_node_t *temp,
  ...
  for (;;) {
    ...
    if (node->key < temp->key) {
      ...
      p = (ngx_memn2cmp(sess_id->id, sess_id_temp->id, (size_t)node->data,
                        (size_t)temp->data) < 0)
              ? &temp->left
              : &temp->right;
    }
...
ngx_int_t ngx_ssl_session_ticket_keys(ngx_conf_t *cf, ngx_ssl_t *ssl,
  ...
  for (i = 0; i < paths->nelts; i++) {
    ...
    if (size != 48 && size != 80) {
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"%V\" must be 48 or 80 bytes",
                         &file.name);
      goto failed;
    }
  ...
  if (SSL_CTX_set_tlsext_ticket_key_cb(
          ssl->ctx, ngx_ssl_session_ticket_key_callback) == 0) {
...
static int ngx_ssl_session_ticket_key_callback(ngx_ssl_conn_t *ssl_conn,
  ...
  found:
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "ssl session ticket decrypt, key: \"%*xs\"%s", (size_t)16,
                   key[i].name, (i == 0) ? " (default)" : "");
...
#else
  {
    ...
    if (altnames) {
      ...
      for (i = 0; i < n; i++) {
        ...
        ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0,
                       "SSL subjectAltName: \"%*s\"", ASN1_STRING_length(str),
    ...
    for (;;) {
      ...
      ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "SSL commonName: \"%*s\"",
...
static char *ngx_openssl_engine(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (engine == NULL) {
    ngx_ssl_error(NGX_LOG_EMERG, cf->log, 0, "ENGINE_by_id(\"%V\") failed",
                  &value[1]);
