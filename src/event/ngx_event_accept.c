...
void ngx_event_accept(ngx_event_t *ev) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ev->log, 0, "accept on %V, ready: %d",
    ...
    *log = ls->log;
...
#if (NGX_DEBUG)
    {
      ...
      if (log->log_level & NGX_LOG_DEBUG_EVENT) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, log, 0, "*%uA accept: %V fd:%d",
                       c->number, &addr, s);
      }
    }
#endif
...
void ngx_debug_accepted_connection(ngx_event_conf_t *ecf, ngx_connection_t *c) {
  ...
  for (i = 0; i < ecf->debug_connection.nelts; i++) {
    ...
    case AF_INET6:
      ...
      for (n = 0; n < 16; n++) {
        if ((sin6->sin6_addr.s6_addr[n] & cidr[i].u.in6.mask.s6_addr[n]) !=
