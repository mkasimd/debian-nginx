...
static void ngx_ssl_stapling_update(ngx_ssl_stapling_t *staple) {
  ...
  if (staple->host.len == 0 || staple->loading ||
      staple->refresh >= ngx_time()) {
    return;
  }
...
ngx_int_t ngx_ssl_ocsp_validate(ngx_connection_t *c) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl ocsp validate, certs:%d",
                 sk_X509_num(ocsp->certs));
...
static void ngx_ssl_ocsp_validate_next(ngx_connection_t *c) {
  ...
  for (;;) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl ocsp validate cert:%ui",
                   ocsp->ncert);
...
static ngx_int_t ngx_ssl_ocsp_responder(ngx_connection_t *c,
  ...
  if (ngx_parse_url(ctx->pool, &u) != NGX_OK) {
    if (u.err) {
      ngx_log_error(NGX_LOG_ERR, c->log, 0,
                    "%s in OCSP responder \"%V\" in certificate", u.err,
                    &u.url);
    }
...
static void ngx_ssl_ocsp_resolve_handler(ngx_resolver_ctx_t *resolve) {
...
#if (NGX_DEBUG)
  {
    ...
    for (i = 0; i < resolve->naddrs; i++) {
      addr.len =
          ngx_sock_ntop(resolve->addrs[i].sockaddr, resolve->addrs[i].socklen,
...
static void ngx_ssl_ocsp_connect(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp connect %ui/%ui",
                 ctx->naddr, ctx->naddrs);
...
static ngx_int_t ngx_ssl_ocsp_create_request(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  escape =
      ngx_escape_uri(NULL, base64.data, base64.len, NGX_ESCAPE_URI_COMPONENT);

  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ctx->log, 0,
                 "ssl ocsp request length %z, escape %d", base64.len,
                 (int)escape);

  len = sizeof("GET ") - 1 + ctx->uri.len + sizeof("/") - 1 + base64.len +
        2 * escape + sizeof(" HTTP/1.0" CRLF) - 1 + sizeof("Host: ") - 1 +
        ctx->host.len + sizeof(CRLF) - 1 + sizeof(CRLF) - 1;
...
static ngx_int_t ngx_ssl_ocsp_process_headers(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  for (;;) {
    ...
    if (rc == NGX_OK) {
      ...
      ngx_log_debug4(
          NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp header \"%*s: %*s\"",
      ...
      if (len == sizeof("Content-Type") - 1 &&
          ngx_strncasecmp(ctx->header_name_start, (u_char *)"Content-Type",
...
static ngx_int_t ngx_ssl_ocsp_verify(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  if (n != OCSP_RESPONSE_STATUS_SUCCESSFUL) {
    ngx_log_error(NGX_LOG_ERR, ctx->log, 0,
                  "OCSP response not successful (%d: %s)", n,
  ...
  if (OCSP_resp_find_status(basic, id, &ctx->status, NULL, NULL, &thisupdate,
                            &nextupdate) != 1) {
    ngx_log_error(NGX_LOG_ERR, ctx->log, 0,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp response, %s, %uz",
...
static ngx_int_t ngx_ssl_ocsp_cache_lookup(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  node = (ngx_ssl_ocsp_cache_node_t *)ngx_str_rbtree_lookup(&cache->rbtree,
                                                            &ctx->key, hash);
...
static ngx_int_t ngx_ssl_ocsp_cache_store(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  node = ngx_slab_calloc_locked(shpool, sizeof(ngx_ssl_ocsp_cache_node_t) +
                                            ctx->key.len);
  if (node == NULL) {
    ...
    if (!ngx_queue_empty(&cache->expire_queue)) {
      ...
      node = ngx_slab_alloc_locked(shpool, sizeof(ngx_ssl_ocsp_cache_node_t) +
                                               ctx->key.len);
    }
...
static ngx_int_t ngx_ssl_ocsp_create_key(ngx_ssl_ocsp_ctx_t *ctx) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp key %xV",
                 &ctx->key);
