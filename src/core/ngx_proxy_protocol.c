...
u_char *ngx_proxy_protocol_read(ngx_connection_t *c, u_char *buf,
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_CORE, c->log, 0,
                 "PROXY protocol src: %V %d, dst: %V %d", &pp->src_addr,
...
invalid:
  ...
  ngx_log_error(NGX_LOG_ERR, c->log, 0, "broken header: \"%*s\"",
                (size_t)(last - buf), buf);
...
static u_char *ngx_proxy_protocol_v2_read(ngx_connection_t *c, u_char *buf,
  ...
  if (version != 2) {
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "unknown PROXY protocol version: %ui",
                  version);
    return NULL;
  }
  ...
  if (transport != 1) {
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0,
                   "PROXY protocol v2 unsupported transport %ui", transport);
    return end;
  }
  ...
  default:
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, c->log, 0,
                   "PROXY protocol v2 unsupported address family %ui", family);
    return end;
  }
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_CORE, c->log, 0,
                 "PROXY protocol v2 src: %V %d, dst: %V %d", &pp->src_addr,
                 pp->src_port, &pp->dst_addr, pp->dst_port);
