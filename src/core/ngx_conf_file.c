...
static ngx_command_t ngx_conf_commands[] = {
    ...
    {ngx_string("include"), NGX_ANY_CONF | NGX_CONF_TAKE1, ngx_conf_include, 0,
     0, NULL},
    ...
    if (b->pos >= b->last) {
      ...
      if (len == NGX_CONF_BUFFER) {
        ...
        if (d_quoted) {
          ...
          ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                             "too long parameter \"%*s...\" started", 10,
                             start);
          return NGX_ERROR;
        }
...
void ngx_cdecl ngx_conf_log_error(ngx_uint_t level, ngx_conf_t *cf,
  ...
  ngx_log_error(level, cf->log, 0, "%*s in %s:%ui", p - errstr, errstr,
...
char *ngx_conf_set_enum_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
  ...
  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid value \"%s\"",
                     value[1].data);

  return NGX_CONF_ERROR;
}

char *ngx_conf_set_bitmask_slot(ngx_conf_t *cf, ngx_command_t *cmd,
                                void *conf) {
  char *p = conf;
  ...
  for (i = 1; i < cf->args->nelts; i++) {
    for (m = 0; mask[m].name.len != 0; m++) {
      ...
      if (*np & mask[m].mask) {
        ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "duplicate value \"%s\"",
                           value[i].data);
...
char *ngx_conf_check_num_bounds(ngx_conf_t *cf, void *post, void *data) {
  ...
  ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "value must be between %i and %i",
