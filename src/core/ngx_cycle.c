...
ngx_cycle_t *ngx_init_cycle(ngx_cycle_t *old_cycle) {
  ...
  if (ngx_test_config) {
    ...
    old_ccf =
        (ngx_core_conf_t *)ngx_get_conf(old_cycle->conf_ctx, ngx_core_module);
  ...
  for (i = 0; /* void */; i++) {
    ...
    file[i].fd =
        ngx_open_file(file[i].name.data, NGX_FILE_APPEND,
                      NGX_FILE_CREATE_OR_OPEN, NGX_FILE_DEFAULT_ACCESS);

    ngx_log_debug3(NGX_LOG_DEBUG_CORE, log, 0, "log: %p %d \"%s\"", &file[i],
                   file[i].fd, file[i].name.data);
  ...
  if (old_cycle->listening.nelts) {
    ...
    for (n = 0; n < cycle->listening.nelts; n++) {
      ...
      for (i = 0; i < old_cycle->listening.nelts; i++) {
        ...
        if (ngx_cmp_sockaddr(nls[n].sockaddr, nls[n].socklen, ls[i].sockaddr,
                             ls[i].socklen, 1) == NGX_OK) {
          nls[n].fd = ls[i].fd;
  ...
  for (i = 0; /* void */; i++) {
    ...
    for (n = 0; /* void */; n++) {
      ...
      if (ngx_strncmp(oshm_zone[i].shm.name.data, shm_zone[n].shm.name.data,
...
ngx_int_t ngx_signal_process(ngx_cycle_t *cycle, char *sig) {
  ...
  if (pid == (ngx_pid_t)NGX_ERROR) {
    ngx_log_error(NGX_LOG_ERR, cycle->log, 0,
                  "invalid PID number \"%*s\" in \"%s\"", n, buf,
                  file.name.data);
    return 1;
  }
...
void ngx_reopen_files(ngx_cycle_t *cycle, ngx_uid_t user) {
  ...
  for (i = 0; /* void */; i++) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "reopen file \"%s\", old:%d new:%d", file[i].name.data,
                   file[i].fd, fd);
    ...
    if (user != (ngx_uid_t)NGX_CONF_UNSET_UINT) {
      ...
      if (ngx_file_info(file[i].name.data, &fi) == NGX_FILE_ERROR) {
        ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno,
                      ngx_file_info_n " \"%s\" failed", file[i].name.data);
...
static void ngx_shutdown_timer_handler(ngx_event_t *ev) {
  ...
  for (i = 0; i < cycle->connection_n; i++) {
    ...
    if (c[i].fd == (ngx_socket_t)-1 || c[i].read == NULL || c[i].read->accept ||
        c[i].read->channel || c[i].read->resolver) {
      continue;
    }

    ngx_log_debug1(NGX_LOG_DEBUG_CORE, ev->log, 0, "*%uA shutdown timeout",
                   c[i].number);
