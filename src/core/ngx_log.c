...
static ngx_command_t ngx_errlog_commands[] = {
    ...
    {ngx_string("error_log"), NGX_MAIN_CONF | NGX_CONF_1MORE, ngx_error_log, 0,
     0, NULL},

    ngx_null_command};

static ngx_core_module_t ngx_errlog_module_ctx = {ngx_string("errlog"), NULL,
                                                  NULL};
...
static ngx_str_t err_levels[] = {
    ngx_null_string,      ngx_string("emerg"), ngx_string("alert"),
...
u_char *ngx_log_errno(u_char *buf, u_char *last, ngx_err_t err) {
  ...
  buf = ngx_slprintf(buf, last,
                     ((unsigned)err < 0x80000000) ? " (%d: " : " (%Xd: ", err);
#else
...
ngx_log_t *ngx_log_init(u_char *prefix, u_char *error_log) {
  ...
  ngx_log_file.fd = ngx_open_file(
      name, NGX_FILE_APPEND, NGX_FILE_CREATE_OR_OPEN, NGX_FILE_DEFAULT_ACCESS);

  if (ngx_log_file.fd == NGX_INVALID_FILE) {
    ngx_log_stderr(ngx_errno,
                   "[alert] could not open error log file: " ngx_open_file_n
                   " \"%s\" failed",
                   name);
#if (NGX_WIN32)
    ngx_event_log(ngx_errno,
                  "could not open error log file: " ngx_open_file_n
                  " \"%s\" failed",
                  name);
#endif
  ...
  for (i = 2; i < cf->args->nelts; i++) {
    ...
    for (n = 1; n <= NGX_LOG_DEBUG; n++) {
      if (ngx_strcmp(value[i].data, err_levels[n].data) == 0) {
        ...
        if (log->log_level != 0) {
          ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "duplicate log level \"%V\"",
                             &value[i]);
    ...
    for (n = 0, d = NGX_LOG_DEBUG_FIRST; d <= NGX_LOG_DEBUG_LAST; d <<= 1) {
      if (ngx_strcmp(value[i].data, debug_levels[n++]) == 0) {
        if (log->log_level & ~NGX_LOG_DEBUG_ALL) {
          ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid log level \"%V\"",
                             &value[i]);
          return NGX_CONF_ERROR;
        }
...
char *ngx_log_set_log(ngx_conf_t *cf, ngx_log_t **head) {
  ...
  if (ngx_strcmp(value[1].data, "stderr") == 0) {
    ...
    needed = sizeof("MEMLOG  :" NGX_LINEFEED) + cf->conf_file->file.name.len +
             NGX_SIZE_T_LEN + NGX_INT_T_LEN + NGX_MAX_ERROR_STR;

    size = ngx_parse_size(&value[1]);

    if (size == (size_t)NGX_ERROR || size < needed) {
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid buffer size \"%V\"",
                         &value[1]);
      return NGX_CONF_ERROR;
    }
    ...
    buf->pos = ngx_slprintf(buf->start, buf->end, "MEMLOG %uz %V:%ui%N", size,
