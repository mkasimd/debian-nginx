...
ngx_resolver_t *ngx_resolver_create(ngx_conf_t *cf, ngx_str_t *names,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (ngx_parse_url(cf->pool, &u) != NGX_OK) {
      if (u.err) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in resolver \"%V\"", u.err,
                           &u.url);
      }
...
void ngx_resolve_name_done(ngx_resolver_ctx_t *ctx) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve name done: %i",
                 ctx->state);
  ...
  if (ctx->state == NGX_AGAIN || ctx->state == NGX_RESOLVE_TIMEDOUT) {
    ...
    if (rn) {
      ...
      ngx_log_error(NGX_LOG_ALERT, r->log, 0, "could not cancel %V resolving",
                    &ctx->name);
    }
  }
...
void ngx_resolve_addr_done(ngx_resolver_ctx_t *ctx) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve addr done: %i",
                 ctx->state);
  ...
  if (ctx->state == NGX_AGAIN || ctx->state == NGX_RESOLVE_TIMEDOUT) {
...

    {
      ...
      addrtext.data = text;
      addrtext.len = ngx_sock_ntop(ctx->addr.sockaddr, ctx->addr.socklen, text,
...
static void ngx_resolver_expire(ngx_resolver_t *r, ngx_rbtree_t *tree,
  ...
  for (i = 0; i < 2; i++) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, r->log, 0, "resolver expire \"%*s\"",
...
static time_t ngx_resolver_resend(ngx_resolver_t *r, ngx_rbtree_t *tree,
  ...
  for (;;) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0, "resolver resend \"%*s\" %p",
...
static void ngx_resolver_process_response(ngx_resolver_t *r, u_char *buf,
  ...
  ngx_log_debug6(NGX_LOG_DEBUG_CORE, r->log, 0,
                 "resolver DNS response %ui fl:%04Xi %ui/%ui/%ud/%ud", ident,
                 flags, nqs, nan, (response->nns_hi << 8) + response->nns_lo,
...
static void ngx_resolver_process_a(ngx_resolver_t *r, u_char *buf, size_t n,
  ...
  if (rn == NULL) {
    ngx_log_error(r->log_level, r->log, 0, "unexpected DNS response for %V",
                  &name);
    ngx_resolver_free(r, name.data);
    goto failed;
  }
  ...
  if (ident != qident) {
    ngx_log_error(r->log_level, r->log, 0,
                  "wrong ident %ui in DNS response for %V, expect %ui", ident,
                  &name, qident);
    ngx_resolver_free(r, name.data);
    goto failed;
  }
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0,
                 "resolver naddrs:%ui cname:%p ttl:%uD", naddrs, cname,
                 rn->ttl);
  ...
  if (cname) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, r->log, 0, "resolver cname:\"%V\"",
                   &name);
...
static void ngx_resolver_process_srv(ngx_resolver_t *r, u_char *buf, size_t n,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0,
                 "resolver nsrvs:%ui cname:%p ttl:%uD", nsrvs, cname, rn->ttl);
  ...
  if (nsrvs) {
    ...
    for (a = 0; a < nan; a++) {
      ...
      if (type == NGX_RESOLVE_SRV) {
        ...
        if (ngx_resolver_copy(r, &srvs[j].name, buf, &buf[i + 6], buf + n) !=
            NGX_OK) {
          goto failed;
        }
...
valid:
  ...
  found:
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_CORE, r->log, 0,
                   "resolver qt:%ui cl:%ui len:%uz", type, class, len);
...
static void ngx_resolver_rbtree_insert_value(ngx_rbtree_node_t *temp,
  ...
  for (;;) {
    ...
    if (node->key < temp->key) {
      ...
      p = (ngx_memn2cmp(rn->name, rn_temp->name, rn->nlen, rn_temp->nlen) < 0)
              ? &temp->left
              : &temp->right;
    }
...
ngx_resolver_rbtree_insert_addr6_value(ngx_rbtree_node_t *temp,
  ...
  for (;;) {
    ...
    if (node->key < temp->key) {
      ...
      p = (ngx_memcmp(&rn->addr6, &rn_temp->addr6, 16) < 0) ? &temp->left
                                                            : &temp->right;
    }
...
static ngx_int_t ngx_resolver_create_name_query(ngx_resolver_t *r,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve: \"%V\" A %i", name,
                 ident & 0xffff);
  ...
  *p = (u_char)len;
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve: \"%V\" AAAA %i", name,
                 ident & 0xffff);
...
static ngx_int_t ngx_resolver_create_srv_query(ngx_resolver_t *r,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_CORE, r->log, 0, "resolve: \"%V\" SRV %i", name,
                 ident & 0xffff);
...
static void ngx_resolver_report_srv(ngx_resolver_t *r,
                                    ngx_resolver_ctx_t *ctx) {
...
static ngx_int_t ngx_udp_connect(ngx_resolver_connection_t *rec) {
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_EVENT, &rec->log, 0, "connect to %V, fd:%d #%uA",
                 &rec->server, s, c->number);
