...
ssize_t ngx_read_file(ngx_file_t *file, u_char *buf, size_t size,
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_CORE, file->log, 0, "read: %d, %p, %uz, %O",
  ...
  if (n == -1) {
    ngx_log_error(NGX_LOG_CRIT, file->log, ngx_errno, "pread() \"%s\" failed",
                  file->name.data);
    return NGX_ERROR;
  }
  ...
  if (file->sys_offset != offset) {
    if (lseek(file->fd, offset, SEEK_SET) == -1) {
      ngx_log_error(NGX_LOG_CRIT, file->log, ngx_errno, "lseek() \"%s\" failed",
                    file->name.data);
      return NGX_ERROR;
    }
  ...
  if (n == -1) {
    ngx_log_error(NGX_LOG_CRIT, file->log, ngx_errno, "read() \"%s\" failed",
                  file->name.data);
    return NGX_ERROR;
  }
...
ssize_t ngx_thread_read(ngx_file_t *file, u_char *buf, size_t size,
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_CORE, file->log, 0,
                 "thread read: %d, %p, %uz, %O", file->fd, buf, size, offset);
  ...
  if (task->event.complete) {
    ...
    if (ctx->err) {
      ngx_log_error(NGX_LOG_CRIT, file->log, ctx->err, "pread() \"%s\" failed",
                    file->name.data);
      return NGX_ERROR;
    }
...
static void ngx_thread_read_handler(void *data, ngx_log_t *log) {
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_CORE, log, 0, "pread: %z (err: %d) of %uz @%O",
...
ssize_t ngx_write_file(ngx_file_t *file, u_char *buf, size_t size,
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_CORE, file->log, 0, "write: %d, %p, %uz, %O",
                 file->fd, buf, size, offset);
  ...
  for (;;) {
    ...
    if (n == -1) {
      ...
      ngx_log_error(NGX_LOG_CRIT, file->log, err, "pwrite() \"%s\" failed",
                    file->name.data);
      return NGX_ERROR;
    }
  ...
  for (;;) {
    ...
    if (n == -1) {
      ...
      ngx_log_error(NGX_LOG_CRIT, file->log, err, "write() \"%s\" failed",
                    file->name.data);
      return NGX_ERROR;
    }
...
ssize_t ngx_write_chain_to_file(ngx_file_t *file, ngx_chain_t *cl, off_t offset,
  ...
  if (cl->next == NULL) {
    return ngx_write_file(file, cl->buf->pos,
                          (size_t)(cl->buf->last - cl->buf->pos), offset);
  }
  ...
  do {
    ...
    if (vec.count == 1) {
      n = ngx_write_file(file, (u_char *)iovs[0].iov_base, iovs[0].iov_len,
                         offset);
...
static ssize_t ngx_writev_file(ngx_file_t *file, ngx_iovec_t *vec,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_CORE, file->log, 0, "writev: %d, %uz, %O",
                 file->fd, vec->size, offset);
...
eintr:
  ...
  if (n == -1) {
    ...
    ngx_log_error(NGX_LOG_CRIT, file->log, err, "pwritev() \"%s\" failed",
                  file->name.data);
    return NGX_ERROR;
  }
...
eintr:
  ...
  if (n == -1) {
    ...
    ngx_log_error(NGX_LOG_CRIT, file->log, err, "writev() \"%s\" failed",
                  file->name.data);
    return NGX_ERROR;
  }
  ...
  if ((size_t)n != vec->size) {
    ngx_log_error(NGX_LOG_CRIT, file->log, 0,
                  "writev() \"%s\" has written only %z of %uz", file->name.data,
                  n, vec->size);
    return NGX_ERROR;
  }
...
ssize_t ngx_thread_write_chain_to_file(ngx_file_t *file, ngx_chain_t *cl,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_CORE, file->log, 0,
                 "thread write chain: %d, %p, %O", file->fd, cl, offset);
...
ngx_int_t ngx_create_file_mapping(ngx_file_mapping_t *fm) {
  ...
  if (ftruncate(fm->fd, fm->size) == -1) {
    ngx_log_error(NGX_LOG_CRIT, fm->log, ngx_errno, "ftruncate() \"%s\" failed",
                  fm->name);
    goto failed;
  }
  ...
  ngx_log_error(NGX_LOG_CRIT, fm->log, ngx_errno, "mmap(%uz) \"%s\" failed",
                fm->size, fm->name);
...
void ngx_close_file_mapping(ngx_file_mapping_t *fm) {
  if (munmap(fm->addr, fm->size) == -1) {
    ngx_log_error(NGX_LOG_CRIT, fm->log, ngx_errno, "munmap(%uz) \"%s\" failed",
                  fm->size, fm->name);
  }
...
size_t ngx_fs_bsize(u_char *name) { return 512; }

off_t ngx_fs_available(u_char *name) { return NGX_MAX_OFF_T_VALUE; }
