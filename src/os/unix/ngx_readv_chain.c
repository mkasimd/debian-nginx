...
ssize_t ngx_readv_chain(ngx_connection_t *c, ngx_chain_t *chain, off_t limit) {
  ...
  if (ngx_event_flags & NGX_USE_KQUEUE_EVENT) {
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "readv: eof:%d, avail:%d, err:%d", rev->pending_eof,
  ...
  if (ngx_event_flags & NGX_USE_EPOLL_EVENT) {
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "readv: eof:%d, avail:%d",
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_EVENT, c->log, 0, "readv: %ui, last:%uz",
  ...
  do {
    ...
    if (n > 0) {
      ...
      if (rev->available >= 0) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "readv: avail:%d",
                       rev->available);
