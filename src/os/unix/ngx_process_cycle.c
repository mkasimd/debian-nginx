...
void ngx_master_process_cycle(ngx_cycle_t *cycle) {
  ...
  for (;;) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "wake up, sigio %i",
                   sigio);
...
static void ngx_signal_worker_processes(ngx_cycle_t *cycle, int signo) {
  ...
  for (i = 0; i < ngx_last_process; i++) {
    ...
    ngx_log_debug7(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "child: %i %P e:%d t:%d d:%d r:%d j:%d", i,
                   ngx_processes[i].pid, ngx_processes[i].exiting,
                   ngx_processes[i].exited, ngx_processes[i].detached,
                   ngx_processes[i].respawn, ngx_processes[i].just_spawn);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, cycle->log, 0, "kill (%P, %d)",
    ...
    if (ngx_processes[i].exited) {
      ...
      if (!ngx_processes[i].detached) {
        ...
        for (n = 0; n < ngx_last_process; n++) {
          ...
          ngx_log_debug3(NGX_LOG_DEBUG_CORE, cycle->log, 0,
                         "pass close channel s:%i pid:%P to:%P", ch.slot,
      ...
      if (ngx_processes[i].respawn && !ngx_processes[i].exiting &&
        ...
        if (ngx_spawn_process(cycle, ngx_processes[i].proc,
                              ...
                              i) == NGX_INVALID_PID) {
          ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "could not respawn %s",
...
static void ngx_worker_process_init(ngx_cycle_t *cycle, ngx_int_t worker) {
  ...
  if (geteuid() == 0) {
    if (setgid(ccf->group) == -1) {
      ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "setgid(%d) failed",
                    ccf->group);
      /* fatal */
      exit(2);
    }
    ...
    if (setuid(ccf->user) == -1) {
      ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "setuid(%d) failed",
                    ccf->user);
      /* fatal */
      exit(2);
    }
...
static void ngx_channel_handler(ngx_event_t *ev) {
  ...
  for (;;) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_CORE, ev->log, 0, "channel command: %ui",
                   ch.command);
    ...
    case NGX_CMD_CLOSE_CHANNEL:
      ...
      ngx_log_debug4(NGX_LOG_DEBUG_CORE, ev->log, 0,
                     "close channel s:%i pid:%P our:%P fd:%d", ch.slot, ch.pid,
                     ngx_processes[ch.slot].pid,
