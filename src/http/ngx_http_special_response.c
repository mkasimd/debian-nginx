...
static u_char ngx_http_error_full_tail[] =
    "<hr><center>" NGINX_VER "</center>" CRLF "</body>" CRLF "</html>" CRLF;

static u_char ngx_http_error_build_tail[] =
    "<hr><center>" NGINX_VER_BUILD "</center>" CRLF "</body>" CRLF
    "</html>" CRLF;
...
ngx_int_t ngx_http_special_response_handler(ngx_http_request_t *r,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "http special response: %i, \"%V?%V\"", error, &r->uri,
                 &r->args);
...
static ngx_int_t ngx_http_send_error_page(ngx_http_request_t *r,
  ...
  return ngx_http_send_special_response(
      r, clcf, r->err_status - NGX_HTTP_MOVED_PERMANENTLY + NGX_HTTP_OFF_3XX);
}
...
static ngx_int_t ngx_http_send_refresh(ngx_http_request_t *r) {
  ...
  size = sizeof(ngx_http_msie_refresh_head) - 1 + escape + len +
