...
ngx_int_t ngx_http_read_client_request_body(
  ...
  if (preread) {
    ...
    if (!r->headers_in.chunked && rb->rest > 0 &&
  ...
  if (r->headers_in.chunked) {
    ...
    for (;;) {
      ...
      if (rc == NGX_AGAIN) {
        ...
        r->headers_in.content_length_n = ngx_max(
            rb->chunked->length, (off_t)cscf->large_client_header_buffers.size);
        break;
      }
...
static ngx_int_t ngx_http_test_expect(ngx_http_request_t *r) {
  ...
  if (r->expect_tested || r->headers_in.expect == NULL ||
  ...
  for (cl = in; cl; cl = cl->next) {
    ...
    for (;;) {
      ...
      ngx_log_debug7(NGX_LOG_DEBUG_EVENT, r->connection->log, 0,
                     ...
                     cl->buf->temporary, cl->buf->in_file, cl->buf->start,
