...
static void ngx_http_process_request_line(ngx_event_t *rev) {
  ...
  for (;;) {
    ...
    if (rc == NGX_OK) {
      ...
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0, "http request line: \"%V\"",
                     &r->request_line);
...
ngx_int_t ngx_http_process_request_uri(ngx_http_request_t *r) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http uri: \"%V\"",
                 &r->uri);

  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http args: \"%V\"",
                 &r->args);
  ...
  for (;;) {
    ...
    if (rc == NGX_OK) {
      ...
      if (r->invalid_header && cscf->ignore_invalid_headers) {
        ...
        ngx_log_error(
            NGX_LOG_INFO, c->log, 0, "client sent invalid header line: \"%*s\"",
...
static ngx_int_t ngx_http_alloc_large_header_buffer(ngx_http_request_t *r,
  ...
  if (r->state != 0 && (size_t)(r->header_in->pos - old) >=
  ...
  if (hc->free) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http large header free: %p %uz", b->pos, b->end - b->last);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http large header alloc: %p %uz", b->pos, b->end - b->last);
...
static ngx_int_t ngx_http_set_virtual_server(ngx_http_request_t *r,
  ...
  if (hc->ssl_servername) {
    if (hc->ssl_servername->len == host->len &&
        ngx_strncmp(hc->ssl_servername->data, host->data, host->len) == 0) {
#if (NGX_PCRE)
...
static ngx_int_t ngx_http_find_virtual_server(
  ...
  cscf = ngx_hash_find_combined(&virtual_names->names,
                                ngx_hash_key(host->data, host->len), host->data,
                                host->len);
...
void ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc) {
  ...
  ngx_log_debug5(NGX_LOG_DEBUG_HTTP, c->log, 0,
                 "http finalize request: %i, \"%V?%V\" a:%d, c:%d", rc, &r->uri,
  ...
  if (r != r->main) {
    ...
    if (r == c->data || r->background) {
      ...
      if (!r->logged) {
        ...
        ngx_log_error(NGX_LOG_ALERT, c->log, 0,
                      "subrequest: \"%V?%V\" logged again", &r->uri, &r->args);
      }
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "http wake parent request: \"%V?%V\"", &pr->uri, &pr->args);
...
static void ngx_http_writer(ngx_http_request_t *r) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, wev->log, 0, "http writer done: \"%V?%V\"",
                 &r->uri, &r->args);
...
static void ngx_http_set_keepalive(ngx_http_request_t *r) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "hc busy: %p %i", hc->busy,
                 hc->nbusy);
...
static void ngx_http_close_request(ngx_http_request_t *r, ngx_int_t rc) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "http request count:%d blk:%d",
                 r->count, r->blocked);
