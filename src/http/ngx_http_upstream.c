...
static ngx_int_t ngx_http_upstream_test_next(ngx_http_request_t *r,
  ...
  if (status == NGX_HTTP_NOT_MODIFIED &&
      u->cache_status == NGX_HTTP_CACHE_EXPIRED && u->conf->cache_revalidate) {
...
static ngx_int_t ngx_http_upstream_process_headers(ngx_http_request_t *r,
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (ngx_hash_find(&u->conf->hide_headers_hash, h[i].hash, h[i].lowcase_key,
                      h[i].key.len)) {
      continue;
    }
...
static void ngx_http_upstream_finalize_request(ngx_http_request_t *r,
  ...
  *u->cleanup = NULL;
  ...
  if (u->pipe && u->pipe->temp_file) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http upstream temp fd: %d", u->pipe->temp_file->file.fd);
  }
...
static char *ngx_http_upstream_server(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (ngx_parse_url(cf->pool, &u) != NGX_OK) {
    if (u.err) {
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in upstream \"%V\"", u.err,
                         &u.url);
    }
...
ngx_http_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags) {
  ...
  for (i = 0; i < umcf->upstreams.nelts; i++) {
    ...
    if ((flags & NGX_HTTP_UPSTREAM_CREATE) &&
      ...
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "duplicate upstream \"%V\"",
                         &u->host);
      return NULL;
    }
