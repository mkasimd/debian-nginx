...
static ngx_int_t ngx_http_file_cache_init(ngx_shm_zone_t *shm_zone,
                                          void *data) {
...
static ngx_int_t ngx_http_file_cache_lock(ngx_http_request_t *r,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "http file cache lock u:%d wt:%M", c->updating, c->wait_time);
...
static ngx_int_t ngx_http_file_cache_read(ngx_http_request_t *r,
  ...
  if ((size_t)h->body_start > c->body_start) {
    ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0,
                  "cache file \"%s\" has too long header", c->file.name.data);
    return NGX_DECLINED;
  }
  ...
  if (c->valid_sec < now) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http file cache expired: %i %T %T", rc, c->valid_sec, now);
...
static ngx_int_t ngx_http_file_cache_exists(ngx_http_file_cache_t *cache,
  ...
  fcn =
      ngx_slab_calloc_locked(cache->shpool, sizeof(ngx_http_file_cache_node_t));
  if (fcn == NULL) {
...
static ngx_int_t ngx_http_file_cache_name(ngx_http_request_t *r,
  ...
  c->file.name.len =
      path->name.len + 1 + path->len + 2 * NGX_HTTP_CACHE_KEY_LEN;
  ...
  for (;;) {
    ...
    if (node->key < temp->key) {
      ...
      p = (ngx_memcmp(cn->key, cnt->key,
                      NGX_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t)) < 0)
              ? &temp->left
              : &temp->right;
    }
...
void ngx_http_file_cache_update(ngx_http_request_t *r, ngx_temp_file_t *tf) {
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "http file cache rename: \"%s\" to \"%s\"", tf->file.name.data,
                 c->file.name.data);
...
void ngx_http_file_cache_update_header(ngx_http_request_t *r) {
  ...
  if (file.fd == NGX_INVALID_FILE) {
    ...
    if (err == NGX_ENOENT) {
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                     "http file cache \"%s\" not found", file.name.data);
      return;
    }
  ...
  if (c->uniq != ngx_file_uniq(&fi) || c->length != ngx_file_size(&fi)) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http file cache \"%s\" changed", file.name.data);
    goto done;
  }
  ...
  if (h.version != NGX_HTTP_CACHE_VERSION ||
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http file cache \"%s\" content changed", file.name.data);
    goto done;
  }
...
void ngx_http_file_cache_free(ngx_http_cache_t *c, ngx_temp_file_t *tf) {
  ...
  if (c->temp_file) {
    if (tf && tf->file.fd != NGX_INVALID_FILE) {
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->file.log, 0,
                     "http file cache incomplete: \"%s\"", tf->file.name.data);

      if (ngx_delete_file(tf->file.name.data) == NGX_FILE_ERROR) {
        ngx_log_error(NGX_LOG_CRIT, c->file.log, ngx_errno,
                      ngx_delete_file_n " \"%s\" failed", tf->file.name.data);
      }
    }
  }
...
static time_t ngx_http_file_cache_forced_expire(ngx_http_file_cache_t *cache) {
  ...
  for (;;) {
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   ...
                   fcn->count, fcn->exists, fcn->key[0], fcn->key[1],
                   fcn->key[2], fcn->key[3]);
...
static ngx_msec_t ngx_http_file_cache_manager(void *data) {
  ...
  for (;;) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                   "http file cache size: %O c:%ui w:%i", size, count,
                   (ngx_int_t)watermark);
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                 "http file cache manager: %ui e:%M n:%M", cache->files,
                 elapsed, next);
...
static void ngx_http_file_cache_loader(void *data) {
  ...
  ngx_log_error(NGX_LOG_NOTICE, ngx_cycle->log, 0,
                "http file cache: %V %.3fM, bsize: %uz", &cache->path->name,
...
static ngx_int_t ngx_http_file_cache_add_file(ngx_tree_ctx_t *ctx,
  ...
  if (ctx->size < (off_t)sizeof(ngx_http_file_cache_header_t)) {
    ngx_log_error(NGX_LOG_CRIT, ctx->log, 0, "cache file \"%s\" is too small",
                  name->data);
    return NGX_ERROR;
  }
...
char *ngx_http_file_cache_set_slot(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  for (i = 2; i < cf->args->nelts; i++) {
    ...
    invalid_levels:
      ...
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid \"levels\" \"%V\"",
                         &value[i]);
      return NGX_CONF_ERROR;
    }
  ...
  if (name.len == 0 || size == 0) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                       "\"%V\" must have \"keys_zone\" parameter", &cmd->name);
    return NGX_CONF_ERROR;
  }
...
char *ngx_http_file_cache_valid_set_slot(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (valid == (time_t)NGX_ERROR) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid time value \"%V\"",
                       &value[n]);
    return NGX_CONF_ERROR;
  }
  ...
  for (i = 1; i < n; i++) {
    ...
    if (ngx_strcmp(value[i].data, "any") == 0) {
      ...
      if (status < 100 || status > 599) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid status \"%V\"",
                           &value[i]);
        return NGX_CONF_ERROR;
      }
    }
