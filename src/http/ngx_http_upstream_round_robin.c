...
#define ngx_http_upstream_tries(p)                                             \
  ((p)->tries + ((p)->next ? (p)->next->tries : 0))
...
ngx_int_t ngx_http_upstream_get_round_robin_peer(ngx_peer_connection_t *pc,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "get rr peer, try: %ui",
                 pc->tries);
  ...
  if (peers->single) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0,
                   "get rr peer, current: %p %i", peer, peer->current_weight);
  }
...
void ngx_http_upstream_free_round_robin_peer(ngx_peer_connection_t *pc,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0, "free rr peer %ui %ui",
                 pc->tries, state);
  ...
  if (state & NGX_PEER_FAILED) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0, "free rr peer failed: %p %i",
...
ngx_http_upstream_set_round_robin_peer_session(ngx_peer_connection_t *pc,
  ...
  if (peers->shpool) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "set session: %p",
                   ssl_session);
...
void ngx_http_upstream_save_round_robin_peer_session(ngx_peer_connection_t *pc,
  ...
  if (peers->shpool) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "save session: %p",
                   ssl_session);
  ...
  if (old_ssl_session) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, pc->log, 0, "old session: %p",
                   old_ssl_session);
