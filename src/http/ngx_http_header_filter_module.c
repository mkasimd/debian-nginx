...
static u_char ngx_http_server_string[] = "Server: KSM-ngx" CRLF;
...
static ngx_int_t ngx_http_header_filter(ngx_http_request_t *r) {
  ...
  if (r->headers_out.location && r->headers_out.location->value.len &&
    ...
    len += sizeof("Location: https://") - 1 + host.len +
  ...
  *b->last++ = LF;
  ...
  if (r->headers_out.content_type.len) {
    b->last =
        ngx_cpymem(b->last, "Content-Type: ", sizeof("Content-Type: ") - 1);
    p = b->last;
  ...
  if (r->headers_out.last_modified == NULL &&
      r->headers_out.last_modified_time != -1) {
    b->last =
        ngx_cpymem(b->last, "Last-Modified: ", sizeof("Last-Modified: ") - 1);
  ...
  if (host.data) {
    ...
    b->last =
        ngx_cpymem(b->last, "Location: http", sizeof("Location: http") - 1);
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "%*s",
