...
static void ngx_http_v2_set_dependency(ngx_http_v2_connection_t *h2c,
                                       ...
                                       ngx_uint_t depend, ngx_uint_t exclusive);
...
static ngx_http_v2_parse_header_t ngx_http_v2_parse_headers[] = {
    {ngx_string("host"), offsetof(ngx_http_headers_in_t, host), 0, NULL},
    ...
    {ngx_string("user-agent"), offsetof(ngx_http_headers_in_t, user_agent), 0,
     NULL},
...
void ngx_http_v2_init(ngx_event_t *rev) {
  ...
  if (ngx_http_v2_send_window_update(
          h2c, 0, NGX_HTTP_V2_MAX_WINDOW - NGX_HTTP_V2_DEFAULT_WINDOW) ==
      NGX_ERROR) {
    ngx_http_close_connection(c);
    return;
  }
...
ngx_int_t ngx_http_v2_send_output_queue(ngx_http_v2_connection_t *h2c) {
  ...
  for (frame = h2c->last_out; frame; frame = fn) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "http2 frame out: %p sid:%ui bl:%d len:%uz", out,
                   out->stream ? out->stream->node->id : 0, out->blocked,
                   out->length);
  }
  ...
  for (/* void */; out; out = fn) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   "http2 frame sent: %p sid:%ui bl:%d len:%uz", out,
                   out->stream ? out->stream->node->id : 0, out->blocked,
                   out->length);
  }
...
static u_char *ngx_http_v2_state_head(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 frame type:%ui f:%Xd l:%uz sid:%ui", type,
                 h2c->state.flags, h2c->state.length, h2c->state.sid);
...
static u_char *ngx_http_v2_state_data(ngx_http_v2_connection_t *h2c,
  ...
  if (h2c->recv_window < NGX_HTTP_V2_MAX_WINDOW / 4) {
    ...
    if (ngx_http_v2_send_window_update(
            h2c, 0, NGX_HTTP_V2_MAX_WINDOW - h2c->recv_window) == NGX_ERROR) {
  ...
  if (size > stream->recv_window) {
    ...
    if (ngx_http_v2_terminate_stream(
            h2c, stream, NGX_HTTP_V2_FLOW_CTRL_ERROR) == NGX_ERROR) {
      return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
    }
  ...
  if (stream->no_flow_control &&
    ...
    if (ngx_http_v2_send_window_update(
            h2c, node->id, NGX_HTTP_V2_MAX_WINDOW - stream->recv_window) ==
        NGX_ERROR) {
      return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
    }
  ...
  if (stream->in_closed) {
    ...
    if (ngx_http_v2_terminate_stream(h2c, stream, NGX_HTTP_V2_STREAM_CLOSED) ==
        NGX_ERROR) {
      return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
    }
...
static u_char *ngx_http_v2_state_field_len(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 %s string, len:%i", huff ? "encoded" : "raw", len);
...
static u_char *ngx_http_v2_state_field_huff(ngx_http_v2_connection_t *h2c,
  ...
  if (ngx_http_v2_huff_decode(&h2c->state.field_state, pos, size,
                              &h2c->state.field_end, h2c->state.field_rest == 0,
...
static u_char *ngx_http_v2_state_process_header(ngx_http_v2_connection_t *h2c,
  ...
  if (ngx_http_v2_validate_header(r, header) != NGX_OK) {
    if (ngx_http_v2_terminate_stream(h2c, h2c->state.stream,
                                     NGX_HTTP_V2_PROTOCOL_ERROR) == NGX_ERROR) {
      return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
    }
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "http2 header: \"%V: %V\"", &header->name, &header->value);
...
static u_char *ngx_http_v2_handle_continuation(ngx_http_v2_connection_t *h2c,
  ...
  if (ngx_http_v2_parse_type(head) != NGX_HTTP_V2_CONTINUATION_FRAME) {
    ngx_log_error(
        NGX_LOG_INFO, h2c->connection->log, 0,
...
static u_char *ngx_http_v2_state_rst_stream(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 RST_STREAM frame, sid:%ui status:%ui", h2c->state.sid,
                 status);
  ...
  case NGX_HTTP_V2_CANCEL:
    ngx_log_error(NGX_LOG_INFO, fc->log, 0, "client canceled stream %ui",
                  h2c->state.sid);
    break;

  case NGX_HTTP_V2_REFUSED_STREAM:
    ngx_log_error(NGX_LOG_INFO, fc->log, 0, "client refused stream %ui",
                  h2c->state.sid);
    break;
...
static u_char *ngx_http_v2_state_ping(ngx_http_v2_connection_t *h2c,
  ...
  frame =
      ngx_http_v2_get_frame(h2c, NGX_HTTP_V2_PING_SIZE, NGX_HTTP_V2_PING_FRAME,
                            NGX_HTTP_V2_ACK_FLAG, 0);
  if (frame == NULL) {
    return ngx_http_v2_connection_error(h2c, NGX_HTTP_V2_INTERNAL_ERROR);
  }
...
static u_char *ngx_http_v2_state_goaway(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 GOAWAY frame: last sid %ui, error %ui", last_sid,
                 error);
#endif
...
static u_char *ngx_http_v2_state_window_update(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 WINDOW_UPDATE frame sid:%ui window:%uz", h2c->state.sid,
                 window);
...
static u_char *ngx_http_v2_state_save(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 frame state save pos:%p end:%p handler:%p", pos, end,
                 handler);
...
ngx_http_v2_stream_t *ngx_http_v2_push_stream(ngx_http_v2_stream_t *parent,
  ...
  for (header = ngx_http_v2_parse_headers; header->name.len; header++) {
    h = (ngx_table_elt_t **)((char *)&parent->request->headers_in +
                             header->offset);
...
rst_stream:
  ...
  if (ngx_http_v2_send_rst_stream(h2c, h2c->last_push,
                                  NGX_HTTP_INTERNAL_SERVER_ERROR) != NGX_OK) {
...
static ngx_int_t ngx_http_v2_send_settings(ngx_http_v2_connection_t *h2c) {
  ...
  *buf->last++ = NGX_HTTP_V2_NO_FLAG;
  ...
  buf->last =
      ngx_http_v2_write_uint16(buf->last, NGX_HTTP_V2_MAX_STREAMS_SETTING);
  buf->last = ngx_http_v2_write_uint32(buf->last, h2scf->concurrent_streams);

  buf->last =
      ngx_http_v2_write_uint16(buf->last, NGX_HTTP_V2_INIT_WINDOW_SIZE_SETTING);
  buf->last = ngx_http_v2_write_uint32(buf->last, h2scf->preread_size);

  buf->last =
      ngx_http_v2_write_uint16(buf->last, NGX_HTTP_V2_MAX_FRAME_SIZE_SETTING);
...
static ngx_int_t ngx_http_v2_send_window_update(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 send WINDOW_UPDATE frame sid:%ui, window:%uz", sid,
                 window);
...
static ngx_int_t ngx_http_v2_send_rst_stream(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 send RST_STREAM frame sid:%ui, status:%ui", sid,
                 status);
...
static ngx_int_t ngx_http_v2_send_goaway(ngx_http_v2_connection_t *h2c,
  ...
  frame =
      ngx_http_v2_get_frame(h2c, NGX_HTTP_V2_GOAWAY_SIZE,
                            NGX_HTTP_V2_GOAWAY_FRAME, NGX_HTTP_V2_NO_FLAG, 0);
  if (frame == NULL) {
    return NGX_ERROR;
  }
...
ngx_http_v2_get_closed_node(ngx_http_v2_connection_t *h2c) {
  ...
  for (q = ngx_queue_head(&node->children);
       q != ngx_queue_sentinel(&node->children); q = ngx_queue_next(q)) {
...
static ngx_int_t ngx_http_v2_validate_header(ngx_http_request_t *r,
  ...
  for (i = (header->name.data[0] == ':'); i != header->name.len; i++) {
    ...
    if (ch == '\0' || ch == LF || ch == CR || ch == ':' ||
        (ch >= 'A' && ch <= 'Z')) {
      ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
                    "client sent invalid header name: \"%V\"", &header->name);
...
static ngx_int_t ngx_http_v2_pseudo_header(ngx_http_request_t *r,
  ...
  ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
                "client sent unknown pseudo-header \":%V\"", &header->name);
...
static ngx_int_t ngx_http_v2_parse_method(ngx_http_request_t *r,
  ...
  do {
    if ((*p < 'A' || *p > 'Z') && *p != '_' && *p != '-') {
      ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
                    "client sent invalid method: \"%V\"", &r->method_name);
...
static ngx_int_t ngx_http_v2_construct_request_line(ngx_http_request_t *r) {
  ...
  if (r->method_name.len == 0 || r->schema.len == 0 ||
  ...
  r->request_line.len =
      r->method_name.len + 1 + r->unparsed_uri.len + sizeof(ending) - 1;
...
static ngx_int_t ngx_http_v2_construct_cookie_header(ngx_http_request_t *r) {
  ...
  h->hash = ngx_hash(
      ngx_hash(ngx_hash(ngx_hash(ngx_hash('c', 'o'), 'o'), 'k'), 'i'), 'e');
...
ngx_int_t ngx_http_v2_read_request_body(ngx_http_request_t *r) {
  ...
  if (stream->in_closed) {
    ...
    if (buf) {
      rc = ngx_http_v2_process_request_body(r, buf->pos, buf->last - buf->pos,
                                            1);
...
ngx_int_t ngx_http_v2_read_unbuffered_request_body(ngx_http_request_t *r) {
  ...
  if (ngx_http_v2_send_window_update(
          h2c, stream->node->id, window - stream->recv_window) == NGX_ERROR) {
    stream->skip_data = 1;
    return NGX_HTTP_INTERNAL_SERVER_ERROR;
  }
    ...
    if (!stream->out_closed) {
      if (ngx_http_v2_send_rst_stream(
              ...
              fc->timedout ? NGX_HTTP_V2_PROTOCOL_ERROR
                           : NGX_HTTP_V2_INTERNAL_ERROR) != NGX_OK) {
        h2c->connection->error = 1;
      }
...
static ngx_int_t ngx_http_v2_adjust_windows(ngx_http_v2_connection_t *h2c,
  ...
  for (i = 0; i < size; i++) {
    ...
    for (node = h2c->streams_index[i]; node; node = node->index) {
      ...
      if (delta > 0 &&
          stream->send_window > (ssize_t)(NGX_HTTP_V2_MAX_WINDOW - delta)) {
        if (ngx_http_v2_terminate_stream(
                h2c, stream, NGX_HTTP_V2_FLOW_CTRL_ERROR) == NGX_ERROR) {
          return NGX_ERROR;
        }
      ...
      ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                     "http2:%ui adjusted window: %z", node->id,
                     stream->send_window);
...
static void ngx_http_v2_set_dependency(ngx_http_v2_connection_t *h2c,
    ...
    if (node->parent != NULL) {
      ...
      for (next = parent->parent;
        ...
        if (node->parent == NGX_HTTP_V2_ROOT) {
          ...
          parent->rel_weight =
              (node->parent->rel_weight / 256) * parent->weight;
        }
  ...
  if (exclusive) {
    for (q = ngx_queue_head(children); q != ngx_queue_sentinel(children);
         q = ngx_queue_next(q)) {
      child = ngx_queue_data(q, ngx_http_v2_node_t, queue);
      child->parent = node;
    }
