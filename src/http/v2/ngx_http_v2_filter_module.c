...
static ngx_chain_t *ngx_http_v2_filter_get_shadow(ngx_http_v2_stream_t *stream,
                                                  ngx_buf_t *buf, off_t offset,
                                                  off_t size);
static ngx_http_v2_out_frame_t *
ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t *stream, size_t len,
...
static ngx_int_t ngx_http_v2_push_frame_handler(ngx_http_v2_connection_t *h2c,
                                                ngx_http_v2_out_frame_t *frame);
...
static ngx_int_t ngx_http_v2_header_filter(ngx_http_request_t *r) {
  ...
  if (!h2c->push_disabled && !h2c->goaway && stream->node->id % 2 == 1 &&
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (header[i].key.len > NGX_HTTP_V2_MAX_FIELD) {
      ngx_log_error(NGX_LOG_CRIT, fc->log, 0,
                    "too long response header name: \"%V\"", &header[i].key);
      return NGX_ERROR;
    }
  ...
  if (r->headers_out.server == NULL) {
    ...
    if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                     "http2 output header: \"server: %s\"", NGINX_VER);

    } else if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_BUILD) {
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                     "http2 output header: \"server: %s\"", NGINX_VER_BUILD);
    ...
    *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_SERVER_INDEX);
    ...
    if (clcf->server_tokens == NGX_HTTP_SERVER_TOKENS_ON) {
      ...
      if (nginx_ver_build[0] == '\0') {
        p = ngx_http_v2_write_value(nginx_ver_build, (u_char *)NGINX_VER_BUILD,
  ...
  if (r->headers_out.date == NULL) {
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                   "http2 output header: \"date: %V\"", &ngx_cached_http_time);
  ...
  if (r->headers_out.last_modified == NULL &&
    ...
    *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_LAST_MODIFIED_INDEX);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                   "http2 output header: \"last-modified: %*s\"", len, pos);
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (fc->log->log_level & NGX_LOG_DEBUG_HTTP) {
      ...
      ngx_log_debug3(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                     "http2 output header: \"%*s: %V\"", header[i].key.len, tmp,
                     &header[i].value);
    }
#endif
    ...
    *pos++ = 0;
    ...
    pos =
        ngx_http_v2_write_name(pos, header[i].key.data, header[i].key.len, tmp);
...
static ngx_int_t ngx_http_v2_push_resource(ngx_http_request_t *r,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2 pushing:%ui limit:%ui", h2c->pushing,
                 h2c->concurrent_pushes);
  ...
  len = (h2c->table_update ? 1 : 0) + 1 + 1 + NGX_HTTP_V2_INT_OCTETS +
        path->len + 1 + NGX_HTTP_V2_INT_OCTETS + r->schema.len;
  ...
  *pos++ = ngx_http_v2_inc_indexed(NGX_HTTP_V2_PATH_INDEX);
  ...
  for (i = 0; i < NGX_HTTP_V2_PUSH_HEADERS; i++) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                   "http2 push header: \"%V: %V\"", &ph[i].name, &(*h)->value);
  ...
  for (;;) {
    ...
    *ll = cl;
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   ...
                   "sid:%ui len:%uz",
                   stream->node->id, frame, h2c->last_push, frame->length);
...
ngx_http_v2_create_trailers_frame(ngx_http_request_t *r) {
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (header[i].key.len > NGX_HTTP_V2_MAX_FIELD) {
      ngx_log_error(NGX_LOG_CRIT, fc->log, 0,
                    "too long response trailer name: \"%V\"", &header[i].key);
      return NULL;
    }
  ...
  for (i = 0; /* void */; i++) {
    ...
    if (fc->log->log_level & NGX_LOG_DEBUG_HTTP) {
      ...
      ngx_log_debug3(NGX_LOG_DEBUG_HTTP, fc->log, 0,
                     "http2 output trailer: \"%*s: %V\"", header[i].key.len,
                     tmp, &header[i].value);
    }
#endif
...
static ngx_chain_t *ngx_http_v2_send_chain(ngx_connection_t *fc,
  ...
  if (in == NULL || stream->out_closed) {
    ...
    if (size) {
      ngx_log_error(NGX_LOG_ERR, fc->log, 0, "output on closed stream");
  ...
  frame_size = (h2lcf->chunk_size < h2c->frame_size) ? h2lcf->chunk_size
                                                     : h2c->frame_size;
...
ngx_http_v2_filter_get_data_frame(ngx_http_v2_stream_t *stream, size_t len,
  ...
  if (buf->start == NULL) {
    buf->start =
        ngx_palloc(stream->request->pool, NGX_HTTP_V2_FRAME_HEADER_SIZE);
    if (buf->start == NULL) {
      return NULL;
    }
  ...
  buf->last =
      ngx_http_v2_write_len_and_type(buf->last, len, NGX_HTTP_V2_DATA_FRAME);
  *buf->last++ = flags;
...
static ngx_inline ngx_int_t ngx_http_v2_flow_control(
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2:%ui windows: conn:%uz stream:%z", stream->node->id,
                 h2c->send_window, stream->send_window);
...
static void ngx_http_v2_waiting_queue(ngx_http_v2_connection_t *h2c,
  ...
  for (q = ngx_queue_last(&h2c->waiting);
       q != ngx_queue_sentinel(&h2c->waiting); q = ngx_queue_prev(q)) {
...
ngx_http_v2_push_frame_handler(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2:%ui PUSH_PROMISE frame %p was sent", stream->node->id,
                 frame);
...
ngx_http_v2_data_frame_handler(ngx_http_v2_connection_t *h2c,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, h2c->connection->log, 0,
                 "http2:%ui DATA frame %p was sent", stream->node->id, frame);
