...
static ngx_int_t ngx_http_init_phases(ngx_conf_t *cf,
                                      ngx_http_core_main_conf_t *cmcf) {
  if (ngx_array_init(&cmcf->phases[NGX_HTTP_POST_READ_PHASE].handlers, cf->pool,
  ...
  if (ngx_array_init(&cmcf->phases[NGX_HTTP_PREACCESS_PHASE].handlers, cf->pool,
                     1, sizeof(ngx_http_handler_pt)) != NGX_OK) {
    return NGX_ERROR;
  }

  if (ngx_array_init(&cmcf->phases[NGX_HTTP_ACCESS_PHASE].handlers, cf->pool, 2,
...
static char *ngx_http_merge_locations(ngx_conf_t *cf, ngx_queue_t *locations,
  ...
  for (q = ngx_queue_head(locations); q != ngx_queue_sentinel(locations);
       q = ngx_queue_next(q)) {
    ...
    rv = ngx_http_merge_locations(cf, clcf->locations, clcf->loc_conf, module,
                                  ctx_index);
    if (rv != NGX_CONF_OK) {
      return rv;
    }
...
static void ngx_http_create_locations_list(ngx_queue_t *locations,
  ...
  for (x = ngx_queue_next(q); x != ngx_queue_sentinel(locations);
       x = ngx_queue_next(x)) {
    lx = (ngx_http_location_queue_t *)x;
...
ngx_http_create_locations_tree(ngx_conf_t *cf, ngx_queue_t *locations,
  ...
  node =
      ngx_palloc(cf->pool, offsetof(ngx_http_location_tree_node_t, name) + len);
  if (node == NULL) {
    return NULL;
  }
  ...
  node->auto_redirect =
      (u_char)((lq->exact && lq->exact->auto_redirect) ||
...
ngx_int_t ngx_http_add_listen(ngx_conf_t *cf, ngx_http_core_srv_conf_t *cscf,
  ...
  if (cmcf->ports == NULL) {
    cmcf->ports =
        ngx_array_create(cf->temp_pool, 2, sizeof(ngx_http_conf_port_t));
    if (cmcf->ports == NULL) {
      return NGX_ERROR;
    }
  }
...
static ngx_int_t ngx_http_add_addresses(ngx_conf_t *cf,
  ...
  for (i = 0; i < port->addrs.nelts; i++) {
    ...
    if (ngx_cmp_sockaddr(lsopt->sockaddr, lsopt->socklen, addr[i].opt.sockaddr,
                         addr[i].opt.socklen, 0) != NGX_OK) {
      continue;
    }
...
static ngx_int_t ngx_http_add_server(ngx_conf_t *cf,
  ...
  if (addr->servers.elts == NULL) {
    ...
    for (i = 0; i < addr->servers.nelts; i++) {
      if (server[i] == cscf) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "a duplicate listen %V",
                           &addr->opt.addr_text);
        return NGX_ERROR;
      }
    }
...
static ngx_int_t ngx_http_add_addrs(ngx_conf_t *cf, ngx_http_port_t *hport,
  ...
  hport->addrs =
      ngx_pcalloc(cf->pool, hport->naddrs * sizeof(ngx_http_in_addr_t));
  if (hport->addrs == NULL) {
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_http_add_addrs6(ngx_conf_t *cf, ngx_http_port_t *hport,
  ...
  hport->addrs =
      ngx_pcalloc(cf->pool, hport->naddrs * sizeof(ngx_http_in6_addr_t));
  if (hport->addrs == NULL) {
    return NGX_ERROR;
  }
  ...
  for (i = 1; i < cf->args->nelts; i++) {
    ...
    for (n = 0; n < (*types)->nelts; n++) {
      ...
      if (ngx_strcmp(value[i].data, type[n].key.data) == 0) {
        ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "duplicate MIME type \"%V\"",
                           &value[i]);
        goto next;
      }
    }
