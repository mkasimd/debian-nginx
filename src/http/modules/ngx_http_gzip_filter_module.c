...
static char *ngx_http_gzip_merge_conf(ngx_conf_t *cf, void *parent,
                                      void *child);
...
static ngx_command_t ngx_http_gzip_filter_commands[] = {
    ...
    {ngx_string("gzip"),
     ...
     offsetof(ngx_http_gzip_conf_t, enable), NULL},
    ...
    {ngx_string("gzip_comp_level"),
     ...
     offsetof(ngx_http_gzip_conf_t, level), &ngx_http_gzip_comp_level_bounds},

    {ngx_string("gzip_window"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_gzip_conf_t, wbits), &ngx_http_gzip_window_p},

    {ngx_string("gzip_hash"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_gzip_conf_t, memlevel), &ngx_http_gzip_hash_p},

    {ngx_string("postpone_gzipping"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_gzip_conf_t, postpone_gzipping), NULL},

    {ngx_string("gzip_no_buffer"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_gzip_conf_t, no_buffer), NULL},

    {ngx_string("gzip_min_length"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_gzip_conf_t, min_length), NULL},
...
static void ngx_http_gzip_filter_memory(ngx_http_request_t *r,
  ...
  if (!ngx_http_gzip_assume_zlib_ng) {
    ctx->allocated = 8192 + 16 + (1 << (wbits + 2)) + (1 << (memlevel + 9));
    ...
    ctx->allocated =
        8192 + 16 + (1 << (wbits + 2)) + 131072 + (1 << (memlevel + 8));
    ctx->zlib_ng = 1;
  }
}
...
static ngx_int_t ngx_http_gzip_filter_add_data(ngx_http_request_t *r,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "gzip in: %p",
                 ctx->in);
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "gzip in_buf:%p ni:%p ai:%ud", ctx->in_buf,
...
static ngx_int_t ngx_http_gzip_filter_deflate(ngx_http_request_t *r,
  ...
  ngx_log_debug6(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 ...
                 ctx->zstream.avail_in, ctx->zstream.avail_out, ctx->flush,
                 ctx->redo);
  ...
  ngx_log_debug5(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 ...
                 ctx->zstream.next_in, ctx->zstream.next_out,
                 ctx->zstream.avail_in, ctx->zstream.avail_out, rc);

  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "gzip in_buf:%p pos:%p", ctx->in_buf, ctx->in_buf->pos);
...
static void *ngx_http_gzip_filter_alloc(void *opaque, u_int items, u_int size) {
  ...
  if (alloc <= ctx->allocated) {
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, ctx->request->connection->log, 0,
                   "gzip alloc: n:%ud s:%ud a:%ui p:%p", items, size, alloc, p);
...
static char *ngx_http_gzip_merge_conf(ngx_conf_t *cf, void *parent,
  ...
  ngx_conf_merge_bufs_value(conf->bufs, prev->bufs, (128 * 1024) / ngx_pagesize,
                            ngx_pagesize);
