...
static char *ngx_http_index_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                           void *child);
...
static ngx_int_t ngx_http_index_handler(ngx_http_request_t *r) {
  ...
  for (i = 0; i < ilcf->indices->nelts; i++) {
    ...
    if (ngx_open_cached_file(clcf->open_file_cache, &path, &of, r->pool) !=
      ...
      if (of.err == NGX_ENOTDIR || of.err == NGX_ENAMETOOLONG ||
          of.err == NGX_EACCES) {
...
static ngx_int_t ngx_http_index_error(ngx_http_request_t *r,
                                      ...
                                      u_char *file, ngx_err_t err) {
  if (err == NGX_EACCES) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, err, "\"%s\" is forbidden",
                  file);
  ...
  if (clcf->log_not_found) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, err, "\"%s\" is not found",
                  file);
  }
