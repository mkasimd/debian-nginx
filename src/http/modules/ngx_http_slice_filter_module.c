...
static ngx_command_t ngx_http_slice_filter_commands[] = {
    ...
    {ngx_string("slice"),
     ...
     offsetof(ngx_http_slice_loc_conf_t, size), NULL},
...
static ngx_int_t ngx_http_slice_header_filter(ngx_http_request_t *r) {
  ...
  if (ctx->etag.len) {
    if (h == NULL || h->value.len != ctx->etag.len ||
        ngx_strncmp(h->value.data, ctx->etag.data, ctx->etag.len) != 0) {
      ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "http slice response range: %O-%O/%O", cr.start, cr.end,
                 cr.complete_length);
  ...
  if (cr.start != ctx->start || cr.end != end) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                  "unexpected range in slice response: %O-%O", cr.start,
                  cr.end);
    return NGX_ERROR;
  }
