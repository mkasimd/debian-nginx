...
static ngx_str_t ngx_http_xslt_default_types[] = {ngx_string("text/xml"),
                                                  ngx_null_string};
...
static ngx_command_t ngx_http_xslt_filter_commands[] = {
    ...
    {ngx_string("xml_entities"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_http_xslt_entities, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("xslt_stylesheet"), NGX_HTTP_LOC_CONF | NGX_CONF_1MORE,
     ngx_http_xslt_stylesheet, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("xslt_param"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE2,
     ngx_http_xslt_param, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("xslt_string_param"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE2,
     ngx_http_xslt_param, NGX_HTTP_LOC_CONF_OFFSET, 0, (void *)1},
    ...
    {ngx_string("xslt_last_modified"),
     ...
     offsetof(ngx_http_xslt_filter_loc_conf_t, last_modified), NULL},
...
static ngx_int_t ngx_http_xslt_body_filter(ngx_http_request_t *r,
                                           ngx_chain_t *in) {
  int wellFormed;
  ngx_chain_t *cl;
  ngx_http_xslt_filter_ctx_t *ctx;
...
static void ngx_http_xslt_sax_external_subset(void *data, const xmlChar *name,
  ...
  if (dtd == NULL) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "xmlCopyDtd() failed");
    return;
  }
...
ngx_http_xslt_apply_stylesheet(ngx_http_request_t *r,
  ...
  for (i = 0; i < conf->sheets.nelts; i++) {
    ...
    res = xsltApplyStylesheetUser(sheet[i].stylesheet, doc, ctx->params.elts,
                                  NULL, NULL, ctx->transform);
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                 "xslt filter type: %d t:%s e:%s", doc_type,
                 type ? type : (u_char *)"(null)",
...
static ngx_int_t ngx_http_xslt_params(ngx_http_request_t *r,
  ...
  for (i = 0; i < params->nelts; i++) {
    ...
    if (param[i].name) {
      ...
      if (param[i].quote) {
        if (xsltQuoteOneUserParam(ctx->transform, param[i].name, string.data) !=
            0) {
          ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
...
static void ngx_http_xslt_cleanup(void *data) { ngx_free(data); }
...
static char *ngx_http_xslt_stylesheet(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (xlcf->sheets.elts == NULL) {
    if (ngx_array_init(&xlcf->sheets, cf->pool, 1,
                       sizeof(ngx_http_xslt_sheet_t)) != NGX_OK) {
      return NGX_CONF_ERROR;
    }
  }
  ...
  if (sheet->stylesheet == NULL) {
    ngx_conf_log_error(NGX_LOG_ERR, cf, 0,
                       "xsltParseStylesheetFile(\"%s\") failed", value[1].data);
    return NGX_CONF_ERROR;
  }
...
found:
  ...
  if (ngx_array_init(&sheet->params, cf->pool, n - 2,
                     sizeof(ngx_http_xslt_param_t)) != NGX_OK) {
    return NGX_CONF_ERROR;
  }
...
static void ngx_http_xslt_cleanup_dtd(void *data) { xmlFreeDtd(data); }
...
static void *ngx_http_xslt_filter_create_main_conf(ngx_conf_t *cf) {
  ...
  if (ngx_array_init(&conf->dtd_files, cf->pool, 1,
                     sizeof(ngx_http_xslt_file_t)) != NGX_OK) {
    return NULL;
  }
...
static char *ngx_http_xslt_filter_merge_conf(ngx_conf_t *cf, void *parent,
  ...
  if (ngx_http_merge_types(cf, &conf->types_keys, &conf->types,
                           ...
                           ngx_http_xslt_default_types) != NGX_OK) {
    return NGX_CONF_ERROR;
  }
