...
static ngx_int_t ngx_http_perl_call_handler(pTHX_ ngx_http_request_t *r,
                                            ngx_http_perl_ctx_t *ctx, HV *nginx,
                                            SV *sub, SV **args,
...
static ngx_command_t ngx_http_perl_commands[] = {
    ...
    {ngx_string("perl_modules"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_str_array_slot, NGX_HTTP_MAIN_CONF_OFFSET,
     offsetof(ngx_http_perl_main_conf_t, modules), NULL},

    {ngx_string("perl_require"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_str_array_slot, NGX_HTTP_MAIN_CONF_OFFSET,
     offsetof(ngx_http_perl_main_conf_t, requires), NULL},

    {ngx_string("perl"), NGX_HTTP_LOC_CONF | NGX_HTTP_LMT_CONF | NGX_CONF_TAKE1,
     ngx_http_perl, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("perl_set"), NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE2,
     ngx_http_perl_set, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
...
static ngx_int_t ngx_http_perl_variable(ngx_http_request_t *r,
...
static ngx_int_t ngx_http_perl_ssi(ngx_http_request_t *r,
...

  {
    ...
    rc = ngx_http_perl_call_handler(aTHX_ r, ctx, pmcf->nginx, sv, asv, handler,
                                    NULL);
...
static ngx_int_t ngx_http_perl_run_requires(pTHX_ ngx_array_t *requires,
  ...
  for (i = 0; i < requires->nelts; i++) {
    ...
    if (SvTRUE(ERRSV)) {
      ...
      ngx_log_error(NGX_LOG_EMERG, log, 0, "require_pv(\"%s\") failed: \"%*s\"",
...
static ngx_int_t ngx_http_perl_call_handler(pTHX_ ngx_http_request_t *r,
  ...
  if (SvTRUE(ERRSV)) {
    ...
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "call_sv(\"%V\") failed: \"%*s\"",
                  handler, len + 1, err);
...
static void ngx_http_perl_eval_anon_sub(pTHX_ ngx_str_t *handler, SV **sv) {
  ...
  if (ngx_strncmp(p, "sub ", 4) == 0 || ngx_strncmp(p, "sub{", 4) == 0 ||
...
static char *ngx_http_perl(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
  ...
  if (plcf->handler.data) {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "duplicate perl handler \"%V\"",
                       &value[1]);
    return NGX_CONF_ERROR;
  }
...

  {
    ...
    if (plcf->sub == &PL_sv_undef) {
      ngx_conf_log_error(NGX_LOG_ERR, cf, 0, "eval_pv(\"%V\") failed",
                         &value[1]);
      return NGX_CONF_ERROR;
    }
