...
static ngx_int_t ngx_http_dav_error(ngx_log_t *log, ngx_err_t err,
                                    ngx_int_t not_found, char *failed,
                                    u_char *path);
static ngx_int_t ngx_http_dav_location(ngx_http_request_t *r);
static void *ngx_http_dav_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_dav_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                         void *child);
...
static ngx_command_t ngx_http_dav_commands[] = {
    ...
    {ngx_string("dav_methods"),
     ...
     offsetof(ngx_http_dav_loc_conf_t, methods), &ngx_http_dav_methods_mask},

    {ngx_string("create_full_put_path"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_dav_loc_conf_t, create_full_put_path), NULL},

    {ngx_string("min_delete_depth"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_num_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_dav_loc_conf_t, min_delete_depth), NULL},

    {ngx_string("dav_access"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE123,
     ngx_conf_set_access_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_dav_loc_conf_t, access), NULL},
...
ok:
  ...
  if (ngx_link_info(path.data, &fi) == NGX_FILE_ERROR) {
    ...
    return ngx_http_dav_error(r->connection->log, err, rc, ngx_link_info_n,
                              path.data);
  }
...
static ngx_int_t ngx_http_dav_delete_path(ngx_http_request_t *r,
  ...
  return ngx_http_dav_error(r->connection->log, ngx_errno, NGX_HTTP_NOT_FOUND,
                            failed, path->data);
}
...
static ngx_int_t ngx_http_dav_delete_file(ngx_tree_ctx_t *ctx,
                                          ngx_str_t *path) {
...
static ngx_int_t ngx_http_dav_mkcol_handler(ngx_http_request_t *r,
  ...
  *(p - 1) = '\0';
  ...
  return ngx_http_dav_error(r->connection->log, ngx_errno, NGX_HTTP_CONFLICT,
...
overwrite_done:
  ...
  if (ngx_link_info(copy.path.data, &fi) == NGX_FILE_ERROR) {
    ...
    if (err != NGX_ENOENT) {
      return ngx_http_dav_error(r->connection->log, err, NGX_HTTP_NOT_FOUND,
                                ngx_link_info_n, copy.path.data);
    }
    ...
    if (ngx_is_dir(&fi) && !slash) {
      ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                    "\"%V\" could not be %Ved to collection \"%V\"", &r->uri,
                    &r->method_name, &dest->value);
      return NGX_HTTP_CONFLICT;
    }
  ...
  if (ngx_link_info(path.data, &fi) == NGX_FILE_ERROR) {
    return ngx_http_dav_error(r->connection->log, ngx_errno, NGX_HTTP_NOT_FOUND,
                              ngx_link_info_n, path.data);
  }

  if (ngx_is_dir(&fi)) {

    if (r->uri.data[r->uri.len - 1] != '/') {
      ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "\"%V\" is collection",
                    &r->uri);
      return NGX_HTTP_BAD_REQUEST;
    }
  ...
  if (ngx_is_dir(&fi)) {
    ...
    if (ngx_create_dir(copy.path.data, ngx_file_access(&fi)) ==
      ...
      return ngx_http_dav_error(r->connection->log, ngx_errno,
                                NGX_HTTP_NOT_FOUND, ngx_create_dir_n,
                                copy.path.data);
    }
...
static ngx_int_t ngx_http_dav_copy_dir(ngx_tree_ctx_t *ctx, ngx_str_t *path) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0, "http copy dir: \"%s\"",
                 path->data);
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0, "http copy dir to: \"%s\"",
                 dir);
...
static ngx_int_t ngx_http_dav_copy_dir_time(ngx_tree_ctx_t *ctx,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0, "http copy dir time: \"%s\"",
                 path->data);
...
static ngx_int_t ngx_http_dav_copy_tree_file(ngx_tree_ctx_t *ctx,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0, "http copy file: \"%s\"",
                 path->data);
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->log, 0, "http copy file to: \"%s\"",
                 file);
