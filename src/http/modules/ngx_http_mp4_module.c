...
#define ngx_mp4_get_64value(p)                                                 \
  (((uint64_t)((u_char *)(p))[0] << 56) +                                      \
   ...
   ((uint64_t)((u_char *)(p))[4] << 24) + (((u_char *)(p))[5] << 16) +         \
...
static ngx_command_t ngx_http_mp4_commands[] = {
    ...
    {ngx_string("mp4"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS, ngx_http_mp4, 0, 0,
     NULL},
    ...
    {ngx_string("mp4_buffer_size"),
     ...
     offsetof(ngx_http_mp4_conf_t, buffer_size), NULL},

    {ngx_string("mp4_max_buffer_size"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_conf_set_size_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_mp4_conf_t, max_buffer_size), NULL},
...
static ngx_int_t ngx_http_mp4_handler(ngx_http_request_t *r) {
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "http mp4 filename: \"%V\"",
                 &path);
...
static ngx_int_t ngx_http_mp4_process(ngx_http_mp4_file_t *mp4) {
  ...
  if (mp4->mdat_atom.buf == NULL) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "no mp4 mdat atom was found in \"%s\"", mp4->file.name.data);
    return NGX_ERROR;
  }
  ...
  *prev = &mp4->mdat_atom;
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "mp4 adjustment:%O",
                 adjustment);
...
static ngx_int_t ngx_http_mp4_read_atom(ngx_http_mp4_file_t *mp4,
  ...
  while (mp4->offset < end) {
    ...
    if (atom_size < sizeof(ngx_mp4_atom_header_t)) {
      ...
      if (atom_size == 1) {
        ...
        if (atom_size < sizeof(ngx_mp4_atom_header64_t)) {
          ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                        "\"%s\" mp4 atom is too small:%uL", mp4->file.name.data,
                        atom_size);
          return NGX_ERROR;
        }
    ...
    ngx_log_debug4(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "mp4 atom: %*s @%O:%uL", (size_t)4, atom_name, mp4->offset,
                   atom_size);

    if (atom_size > (uint64_t)(NGX_MAX_OFF_T_VALUE - mp4->offset) ||
        mp4->offset + (off_t)atom_size > end) {
      ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                    "\"%s\" mp4 atom too large:%uL", mp4->file.name.data,
                    atom_size);
      return NGX_ERROR;
    }
...
static ngx_int_t ngx_http_mp4_read(ngx_http_mp4_file_t *mp4, size_t size) {
  ...
  if (mp4->buffer_size < size) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0, "\"%s\" mp4 file truncated",
                  mp4->file.name.data);
    return NGX_ERROR;
  }
  ...
  if ((size_t)n != mp4->buffer_size) {
    ngx_log_error(NGX_LOG_CRIT, mp4->file.log, 0,
                  ngx_read_file_n " read only %z of %z from \"%s\"", n,
                  mp4->buffer_size, mp4->file.name.data);
    return NGX_ERROR;
  }
...
static ngx_int_t ngx_http_mp4_read_ftyp_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (atom_data_size > 1024 ||
    ...
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "\"%s\" mp4 ftyp atom is too large:%uL", mp4->file.name.data,
                  atom_data_size);
    return NGX_ERROR;
  }
   ...
   */
  ngx_memcpy(ftyp_atom + sizeof(ngx_mp4_atom_header_t), ngx_mp4_atom_data(mp4),
...
static ngx_int_t ngx_http_mp4_read_moov_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (atom_data_size > mp4->buffer_size) {
    ...
    mp4->buffer_size =
        (size_t)atom_data_size + NGX_HTTP_MP4_MOOV_BUFFER_EXCESS * no_mdat;
  }
...
static size_t ngx_http_mp4_update_mdat_atom(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "mdat new offset @%O:%O",
                 start_offset, atom_data_size);
...
static ngx_int_t ngx_http_mp4_read_mvhd_atom(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "mvhd timescale:%uD, duration:%uL, time:%.3fs", timescale,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "mvhd new duration:%uL, time:%.3fs", duration,
...
static ngx_int_t ngx_http_mp4_read_tkhd_atom(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "tkhd duration:%uL, time:%.3fs", duration,
                 (double)duration / mp4->timescale);
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "tkhd new duration:%uL, time:%.3fs", duration,
                 (double)duration / mp4->timescale);
...
static ngx_int_t ngx_http_mp4_read_mdhd_atom(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "mdhd timescale:%uD, duration:%uL, time:%.3fs", timescale,
                 duration, (double)duration / timescale);
  ...
  ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "mdhd new duration:%uL, time:%.3fs", duration,
                 (double)duration / timescale);
...
static void ngx_http_mp4_update_minf_atom(ngx_http_mp4_file_t *mp4,
  ...
  trak->size += sizeof(ngx_mp4_atom_header_t) + trak->vmhd_size +
                trak->smhd_size + trak->dinf_size;
  atom = &trak->minf_atom_buf;
  ngx_mp4_set_32value(atom->pos, trak->size);
}
...
static ngx_int_t ngx_http_mp4_read_stsd_atom(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 ...
                 ngx_mp4_get_32value(stsd_atom->entries), (size_t)4,
...
static ngx_int_t ngx_http_mp4_read_stts_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (ngx_mp4_atom_data_size(ngx_mp4_stts_atom_t) +
          entries * sizeof(ngx_mp4_stts_entry_t) >
      atom_data_size) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "\"%s\" mp4 stts atom too small", mp4->file.name.data);
    return NGX_ERROR;
...
static ngx_int_t ngx_http_mp4_crop_stts_data(ngx_http_mp4_file_t *mp4,
  ...
  while (entry < end) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "time:%uL, count:%uD, duration:%uD", start_time, count,
                   duration);
  ...
  if (start) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "end_sample:%ui",
                   trak->end_sample);
...
found:
  ...
  if (start) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "start_sample:%ui, new count:%uD", trak->start_sample,
                   count - rest);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "end_sample:%ui, new count:%uD", trak->end_sample, rest);
  }
...
static ngx_int_t ngx_http_mp4_read_ctts_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (ngx_mp4_atom_data_size(ngx_mp4_ctts_atom_t) +
          entries * sizeof(ngx_mp4_ctts_entry_t) >
      atom_data_size) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "\"%s\" mp4 ctts atom too small", mp4->file.name.data);
    return NGX_ERROR;
...
static void ngx_http_mp4_crop_ctts_data(ngx_http_mp4_file_t *mp4,
  ...
  while (entry < end) {
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "sample:%uD, count:%uD, offset:%uD", start_sample, count,
...
static ngx_int_t ngx_http_mp4_read_stsc_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (ngx_mp4_atom_data_size(ngx_mp4_stsc_atom_t) +
          entries * sizeof(ngx_mp4_stsc_entry_t) >
      atom_data_size) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "\"%s\" mp4 stsc atom too small", mp4->file.name.data);
    return NGX_ERROR;
...
static ngx_int_t ngx_http_mp4_update_stsc_atom(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "sample-to-chunk entries:%uD", trak->sample_to_chunk_entries);
...
static ngx_int_t ngx_http_mp4_crop_stsc_data(ngx_http_mp4_file_t *mp4,
  ...
  ngx_log_debug4(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                 "sample:%uD, chunk:%uD, chunks:%uD, samples:%uD", start_sample,
                 chunk, next_chunk - chunk, samples);
...
found:
  ...
  if (samples == 0) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "zero number of samples in \"%s\"", mp4->file.name.data);
    return NGX_ERROR;
  }
  ...
  if (start) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0,
                   "end_chunk:%ui, end_chunk_samples:%ui", trak->end_chunk,
...
static ngx_int_t ngx_http_mp4_read_stco_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (ngx_mp4_atom_data_size(ngx_mp4_stco_atom_t) + entries * sizeof(uint32_t) >
      atom_data_size) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "\"%s\" mp4 stco atom too small", mp4->file.name.data);
    return NGX_ERROR;
...
static ngx_int_t ngx_http_mp4_update_stco_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (chunk_offset > (uint64_t)mp4->end - samples_size ||
    ...
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "too large chunk offset in \"%s\"", mp4->file.name.data);
    return NGX_ERROR;
  }
  ...
  ngx_log_debug1(NGX_LOG_DEBUG_HTTP, mp4->file.log, 0, "start chunk offset:%O",
                 trak->start_offset);
...
static ngx_int_t ngx_http_mp4_read_co64_atom(ngx_http_mp4_file_t *mp4,
  ...
  if (ngx_mp4_atom_data_size(ngx_mp4_co64_atom_t) + entries * sizeof(uint64_t) >
      atom_data_size) {
    ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
                  "\"%s\" mp4 co64 atom too small", mp4->file.name.data);
    return NGX_ERROR;
...
static char *ngx_http_mp4_merge_conf(ngx_conf_t *cf, void *parent,
                                     void *child) {
