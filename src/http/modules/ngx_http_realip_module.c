...
static char *ngx_http_realip_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                            void *child);
...
static ngx_int_t ngx_http_realip_remote_addr_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_realip_remote_port_variable(
    ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_command_t ngx_http_realip_commands[] = {
    ...
    {ngx_string("set_real_ip_from"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_http_realip_from, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("real_ip_header"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
         NGX_CONF_TAKE1,
     ngx_http_realip, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("real_ip_recursive"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_realip_loc_conf_t, recursive), NULL},
...
static ngx_int_t ngx_http_realip_handler(ngx_http_request_t *r) {
  ...
  default: /* NGX_HTTP_REALIP_HEADER */
    ...
    for (i = 0; /* void */; i++) {
      ...
      if (hash == header[i].hash && len == header[i].key.len &&
...
static ngx_int_t ngx_http_realip_set_addr(ngx_http_request_t *r,
  ...
  len = ngx_sock_ntop(addr->sockaddr, addr->socklen, text, NGX_SOCKADDR_STRLEN,
                      0);
  if (len == 0) {
    return NGX_HTTP_INTERNAL_SERVER_ERROR;
  }
...
static char *ngx_http_realip_from(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (ngx_inet_resolve_host(cf->pool, &u) != NGX_OK) {
    if (u.err) {
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "%s in set_real_ip_from \"%V\"",
                         u.err, &u.host);
    }
