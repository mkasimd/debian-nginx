...
static char *ngx_http_rewrite_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                             void *child);
...
static ngx_command_t ngx_http_rewrite_commands[] = {
    ...
    {ngx_string("rewrite"),
     NGX_HTTP_SRV_CONF | NGX_HTTP_SIF_CONF | NGX_HTTP_LOC_CONF |
         NGX_HTTP_LIF_CONF | NGX_CONF_TAKE23,
     ngx_http_rewrite, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("return"),
     NGX_HTTP_SRV_CONF | NGX_HTTP_SIF_CONF | NGX_HTTP_LOC_CONF |
         NGX_HTTP_LIF_CONF | NGX_CONF_TAKE12,
     ngx_http_rewrite_return, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},

    {ngx_string("break"),
     NGX_HTTP_SRV_CONF | NGX_HTTP_SIF_CONF | NGX_HTTP_LOC_CONF |
         NGX_HTTP_LIF_CONF | NGX_CONF_NOARGS,
    ...
    {ngx_string("rewrite_log"),
     ...
     offsetof(ngx_http_rewrite_loc_conf_t, log), NULL},

    {ngx_string("uninitialized_variable_warn"),
     NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_SIF_CONF |
         NGX_HTTP_LOC_CONF | NGX_HTTP_LIF_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_HTTP_LOC_CONF_OFFSET,
     offsetof(ngx_http_rewrite_loc_conf_t, uninitialized_variable_warn), NULL},
...
static ngx_int_t ngx_http_rewrite_var(ngx_http_request_t *r,
...
static char *ngx_http_rewrite(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
  ...
  regex_end = ngx_http_script_add_code(
      lcf->codes, sizeof(ngx_http_script_regex_end_code_t), &regex);
  if (regex_end == NULL) {
    return NGX_CONF_ERROR;
  }
  ...
  regex->next =
      (u_char *)lcf->codes->elts + lcf->codes->nelts - (u_char *)regex;
...
static char *ngx_http_rewrite_return(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (ret->status == (uintptr_t)NGX_ERROR) {
    ...
    if (cf->args->nelts == 2 &&
      ...
      ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid return code \"%V\"",
                         &value[1]);
      return NGX_CONF_ERROR;
    }
...
static char *ngx_http_rewrite_if(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  *cf = save;
  ...
  if (elts != lcf->codes->elts) {
    if_code =
        (ngx_http_script_if_code_t *)((u_char *)if_code +
                                      ((u_char *)lcf->codes->elts - elts));
  }

  if_code->next =
      (u_char *)lcf->codes->elts + lcf->codes->nelts - (u_char *)if_code;
...
static char *ngx_http_rewrite_if_condition(ngx_conf_t *cf,
  ...
  if (value[1].len < 1 || value[1].data[0] != '(') {
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid condition \"%V\"",
                       &value[1]);
    return NGX_CONF_ERROR;
  }
  ...
  if (len > 1 && p[0] == '$') {
    ...
    if (len == 1 && p[0] == '=') {
      ...
      code =
          ngx_http_script_start_code(cf->pool, &lcf->codes, sizeof(uintptr_t));
      if (code == NULL) {
        return NGX_CONF_ERROR;
      }
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "unexpected \"%V\" in condition",
                       &value[cur]);
    return NGX_CONF_ERROR;
...
static char *ngx_http_rewrite_set(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (v->set_handler) {
    vhcode = ngx_http_script_start_code(
        cf->pool, &lcf->codes, sizeof(ngx_http_script_var_handler_code_t));
    if (vhcode == NULL) {
      return NGX_CONF_ERROR;
    }
...
static char *ngx_http_rewrite_value(ngx_conf_t *cf,
  ...
  complex = ngx_http_script_start_code(
      cf->pool, &lcf->codes, sizeof(ngx_http_script_complex_value_code_t));
  if (complex == NULL) {
    return NGX_CONF_ERROR;
  }
