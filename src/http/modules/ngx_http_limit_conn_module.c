...
static ngx_int_t ngx_http_limit_conn_status_variable(
...
static ngx_command_t ngx_http_limit_conn_commands[] = {
    ...
    {ngx_string("limit_conn_dry_run"),
     ...
     offsetof(ngx_http_limit_conn_conf_t, dry_run), NULL},
...
static ngx_http_variable_t ngx_http_limit_conn_vars[] = {
    ...
    {ngx_string("limit_conn_status"), NULL, ngx_http_limit_conn_status_variable,
  ...
  for (i = 0; i < lccf->limits.nelts; i++) {
    ...
    if (node == NULL) {
      ...
      n = offsetof(ngx_rbtree_node_t, color) +
          offsetof(ngx_http_limit_conn_node_t, data) + key.len;
...
static ngx_int_t ngx_http_limit_conn_init_zone(ngx_shm_zone_t *shm_zone,
                                               void *data) {
...
static char *ngx_http_limit_conn(ngx_conf_t *cf, ngx_command_t *cmd,
  ...
  if (limits == NULL) {
    if (ngx_array_init(&lccf->limits, cf->pool, 1,
                       sizeof(ngx_http_limit_conn_limit_t)) != NGX_OK) {
      return NGX_CONF_ERROR;
    }
  }
